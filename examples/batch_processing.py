...
def main():
    ...
    # Initialisation dominates one lookup, so reuse the instance for the whole batch.
    with TimezoneFinder(in_memory=True) as tf:
        ...
        # "skip" answers valid points instead of discarding their completed work.
        names = tf.timezone_names_at(
            lngs=spoilt_lngs, lats=spoilt_lats, on_invalid="skip"
        )
        ids = tf.timezone_ids_at(lngs=spoilt_lngs, lats=spoilt_lats, on_invalid="skip")
        print(f"  names: {names}")
        print(f"  ids:   {ids} ({NO_ZONE_ID} means no scalar answer)")
