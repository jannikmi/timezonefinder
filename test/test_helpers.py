...
class HelperTest(unittest.TestCase):
    from timezonefinder import helpers
    ...
    def test_inside_polygon(self):
        ...
        print(
            template.format("#test poly", "#test point", "EXPECTED", "COMPUTED", "  ")
        )
        print("=" * 50)
    ...
    def test_rectify_coords(self):
        ...
        for inp, expected_output in test_cases:
            ...
            ), f"results do not match: {output} != {expected_output}"
    ...
    def test_all_the_same(self):
        ...
        for pointer, length, id_list in test_cases:
            ...
            assert (
                all_the_same_fct(
                    pointer, length, np.array(id_list, dtype=DTYPE_FORMAT_H_NUMPY)
                )
                == expected_results[i]
            )
            i += 1
    ...
    def test_distance_computation(self):
        ...
        NR_TEST_RUNS = 1000
        for _ in range(NR_TEST_RUNS):
            rnd_point = random_point()
            lng_rnd_point2 = random_point()[0]
            hav_result = degrees(
                haversine(
                    radians(rnd_point[0]), radians(rnd_point[1]), lng_rnd_point2, 0
                )
            )
            result = degrees(
                distance_to_point_on_equator(
                    radians(rnd_point[0]), radians(rnd_point[1]), lng_rnd_point2
                )
            )
