...
**One event closing several sibling entries is one entry, written once.** When a single decision retires a family of slices, its shared reason, measurement and reopen condition belong in the canonical record that decision came from, and the siblings are consolidated in that same change rather than left as N withdrawn entries each carrying a copy. That duplication is invisible in review, because every entry still reads as complete on its own; the test is per fact, not per file — a fact that would have to be corrected in more than one entry at once is already in the wrong place. A closed entry then keeps only what stops a re-proposal: the dead end, the evidence that killed it, and what would revive it — never the location, fix, changelog sentence or sequencing a live item needs.
