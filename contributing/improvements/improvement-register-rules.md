...
**A confirmed defect outranks everything that is not one.** Decided 2026-08-29 by the maintainer: a correctness defect goes to the top the moment it is confirmed, and a pass takes it before any improvement, whatever that improvement's size or expected value. The ordering above governs what sits *below* the defects. Two boundaries keep it honest — a **suspected** defect is a measurement rather than a defect, so confirm it before it displaces anything; and a confirmed defect that is blocked still sits below its blocker. Stated as an absolute because a narrow defect prices low on any honest expected-value reading — few users reach it, the fix is small, nothing waits on it — which is exactly how real wrong answers end up ranked beneath refactors.
