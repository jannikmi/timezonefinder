...
```
FMT-1 (latitude block index) ─→ [data 2.x release] ─→ DATA-BINARIES ─→ GH-522
        (the held format-2 window)                      (stop committing)  [strictly after]

FMT-1 ─→ FMT-2 (frame-of-reference payload) ─→ [a format 3, shared with GH-513]
...
- **FMT-1 sequences *before* DATA-BINARIES, not after — the maintainer held the 2.x release open so the format change rides format 2.** The older ordering ran the other way, on the ground that uncommitting the binaries first makes every later regeneration cheap in history. That reasoning still holds and is simply outweighed: ~61 MiB of pack growth is recoverable by GH-522, whereas a second ordered two-distribution release is not, and waiting for DATA-BINARIES *guarantees* the extra release because DATA-BINARIES cannot start until the very publication that shuts the window. **What rides the window is FMT-1 alone, decided 2026-08-30**: it is the small additive half and it carries the whole query win, so the hold does not have to cover a re-encoding of 63 MB. FMT-2 therefore takes a format 3 — deliberately, and priced in its entry. The [format-batching decisions](decisions/data-format-version-marker-and-release-batching-decisions.md) carry the mechanics and the exit condition.
