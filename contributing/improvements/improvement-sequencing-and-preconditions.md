...
```
DATA-BINARIES: (1) artifact hand-off ─→ (2) git-ignore `data/` ─→ GH-522
  (bootstrap half shipped; the two steps are ordered, additive before subtractive)
...
- **DATA-BINARIES is ordered internally, additive before subtractive.** `timezonefinder-data 2.2026.3` published on 2026-08-30 and the bootstrap half shipped against it. What is left is two steps that may not be swapped: the release-side artifact hand-off first, because it is additive and can land while the binaries are still committed, and only then git-ignoring `data/` — which is what makes every checkout depend on the hand-off already existing. Its entry carries both.
