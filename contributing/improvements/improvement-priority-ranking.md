...
| [FT-2](items/data-pipeline-and-developer-tooling/ft-2-no-environment-tests-a-free-threaded-interpreter.md) | No environment tests a free-threaded interpreter | testing | ~20 | free — `pytest-run-parallel` is installed and dormant; the strict-xfail GIL assertion is what FT-3 flips |
| [FT-4](items/data-pipeline-and-developer-tooling/ft-4-the-thread-safety-documentation-contradicts-itself.md) | The thread-safety documentation contradicts itself | docs | S | free — two published pages state opposites; the code and `tests/test_resource_management.py` already say which is wrong |
