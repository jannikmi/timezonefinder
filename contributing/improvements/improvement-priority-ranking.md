...
| [TOOL-3](items/data-pipeline-and-developer-tooling/tool-3-b905-wants-an-explicit-strict-on-27-zip-sites.md) | `B905` wants an explicit `strict=` on 27 `zip` sites | tooling | ~40 | free — 27 sites, each needing its own length judgement |
...
| [FMT-3](items/lookup-geometry-and-data-format/fmt-3-the-payload-offset-width-is-justified-by-a-bound-that-is-not-the-one-it-holds.md) | The payload-offset dtype is justified by a bound it does not hold | data format | ~30 | free — no answer is wrong today |
| [DOC-7](items/data-pipeline-and-developer-tooling/doc-7-move-the-readme-to-markdown-with-semantic-line-breaks.md) | Move `README.rst` to Markdown, reflowing it in the same pass | docs | ~200 | free — decided 2026-09-05; changes the published PyPI page, and takes the README out of DOC-5 |
...
| [DOC-5](items/data-pipeline-and-developer-tooling/doc-5-convert-the-remaining-prose-pages-to-semantic-line-breaks.md) | Convert the remaining prose pages to semantic line breaks | docs | ~157 | free — take when nothing else is queued; seven small files, the README having moved to DOC-7 |
| [DOC-6](items/data-pipeline-and-developer-tooling/doc-6-price-a-check-that-rejects-a-new-hard-wrapped-paragraph.md) | Price a check that rejects a newly hard-wrapped paragraph | docs tooling | S | blocked on DOC-2 to DOC-5 and DOC-7 — and may close with a measurement rather than a check |
