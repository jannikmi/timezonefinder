...
- **Where:** `pyproject.toml`, the `h3>=4` dependency bound and `uv.lock`; the strict-xfail assertion in `tests/test_free_threading.py`, and `--ignore-gil-enabled` on the parallel command of `tox.ini`'s `free-threaded` env, which goes with it; `docs/`, for the support statement.
...
- **Blocked on an h3 release, and the fix is merged but unshipped.** uber/h3-py#493 merged 2026-08-12; the newest h3 release is 4.5.0, published 2026-05-30, which predates it. `uv.lock` holds 4.5.0, and constructing a `TimezoneFinder` on a free-threaded build still emits the runtime warning that the GIL was re-enabled to load `h3._cy.cells` — re-verified 2026-09-08. A bare `import timezonefinder` does not; importing the `TimezoneFinder` class does, which is why the assertion imports and uses it — in a subprocess, since the GIL never turns back off within one process. **The next h3 release is the thing to re-check** — do not read this as waiting for a release that has already happened.
...
- **What it must not claim:** that a shared instance now scales. It does not — 1.60x at 8 threads shared against 4.84x per-thread — and that is refcount contention, not a lock. The published docs already say so: one instance per thread is recommended for throughput, not correctness, so an h3 fix changes no sentence there.
...
- **Size:** S.
- **Status:** blocked on an h3 release carrying uber/h3-py#493.
- **Detail:** issue #364.
