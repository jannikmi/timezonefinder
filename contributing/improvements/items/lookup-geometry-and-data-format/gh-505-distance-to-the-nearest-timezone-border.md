...
- **Signal check, 2026-08-20: none.** Zero reactions and zero third-party comments; the only comment is the maintainer's own note that a `closest_timezone_at` existed historically and the history is worth scanning first. Re-checking costs one `gh issue view 505` and is the whole of what a pass should do here.
...
- **Last touched:** 2026-08-20 — signal checked and recorded, so the next pass can see when it was last looked at rather than re-deciding whether to look.
