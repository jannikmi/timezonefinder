...
- `prototypes/shortcut_ordering_optimum.py` — the experiment itself, re-runnable and extensible; its `FINDINGS` block carries the same numbers with the derivations behind them.
...
- **The `unique` control drifted toward the challenger** on three runs (37-47 of 61 rounds, `unresolved`), so the ambiguous losses are if anything understated.
...
**The model.** `E = Σ_{t<S} c_t · (1 − Σ_{u<t} p_u)`, with `S` the index where the final zone's run starts, `c_t` the expected compute of a test and `p_t` its hit probability for a uniform point. Zone contiguity is a hard constraint: `last_zone_change_idx` needs one final run.

**The construction.** Within a zone, order by `c/p` ascending — adjacent exchange moves `E` by `c_j·p_i − c_i·p_j`. Across zones, a zone entered at survival `R` contributes `R·C − K` with `C = Σc`, `K = Σ_i c_i·Σ_{u<i} p_u`, so exchanging adjacent blocks moves `E` by `C_A·P_B − C_B·P_A`: the `K` terms cancel and a zone behaves **exactly** as one job of cost `C` and probability `P`, ordered by `C/P` ascending. The free final slot is not a sort key — designating `m` last both saves its contribution and removes its probability from shielding what follows, `E(m) = E_all − (R_m·C_m − K_m) + P_m·Σ_{j>m} C_j` — so with `k ≤ 25` zones evaluate all `k`.
...
**The cost coefficient is not the vertex count**, and this is the part with teeth. The kernel skips latitude blocks, so `c_i = α + β·blocks + γ·in-band edges`, every count computable at build time from `_block_index.block_latitude_ranges`. Calibrated against measured `PolygonArray.pip` times over 1,500 samples: **`437.5 ns + 0.511·blocks + 1.133·in-band edges`, R² = 0.9886**, against **R² = 0.524 for the total vertex count** the shipped key sorts on. A ring of ≥512 vertices scans a median **2.2 %** of itself, and the two keys disagree on which candidate is cheaper in **31.6 % of pairs**.

**And it still does not matter, because the loop is overhead-bound.** With the calibrated model, expected candidate-loop compute per ambiguous query is 758.5 ns shipped, 735.8 ns under a pure true-cost key and **723.8 ns under the optimum — −4.57 %**. But 437.5 ns of that is fixed per-candidate overhead and only ~1.05 candidates are tested, so the loop is **14.7 % of a 5,150 ns ambiguous query**: the optimum saves **34.7 ns, 0.67 % of an ambiguous query and 0.27 % of a mixed workload**. The A/B agrees: built from the calibrated constants it is **no difference on every stratum**, both estimators agreeing — +0.1 % on ambiguous with 30 of 61 rounds won, and the `unique` control equally still. Earlier runs against an index built from guessed constants read `unresolved` and swung +3.8 / −6.3 / +0.2 %; calibrating is what made the comparison quiet. **Even an oracle** that knows the answer and pays only the cheapest candidate saves 80 % of the loop, which is 11.8 % of an ambiguous query and **4.7 % of a mixed workload** — that is the ceiling on candidate ordering as a subject, reached only by clairvoyance.
...
- **Rebuilding the shipped ordering from source reproduces the packaged binary to 29 bytes**, across the handful of cells where the sort key ties and `polys_in_cell`'s set iteration order breaks the tie. Recorded separately as TOOL-7; the fix is a polygon-id tie-break and needs neither area nor a dependency.

## Reproducing it

`prototypes/shortcut_ordering_optimum.py` is the whole experiment: the model and its assumptions, every ordering, the construction, the brute-force check, the cost calibration and the A/B, as cached stages. Adding an ordering is one function in its `STRATEGIES` table. Re-run `enumerate` after any H3 resolution or dataset change, since the bound moves with both.
...
Taken against `c27b452` on Apple arm64, free-threaded CPython 3.14.2, data 2026c, fixture set v3.
