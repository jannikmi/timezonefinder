...
- **It must not discard the candidates `timezones_at` reads.** That method walks an ambiguous cell's whole candidate list and reports every zone containing the point; the dataset's overlaps are real, and `Asia/Urumqi` inside `Asia/Shanghai` is most of them. Collapsing a covered ambiguous cell into a unique-zone entry throws the other zones' candidates away, so exactly the cells this item converts are where that method would silently answer one zone where it answers two — and an under-reported list still reads as plausible, so review will not catch it. **Nor will a green suite**, and the reason is worth knowing: `tests/test_overlapping_zones.py` compares the method against a full candidate scan, but that scan enumerates the *same* index, so a converted cell degrades both sides and they keep agreeing. The regression surface is that file's `DOCUMENTED_OVERLAPS` — six hard-coded coordinates asserted as exact lists — of which `Asia/Urumqi` over `Asia/Shanghai` is both the first and one of the two pairs this conversion rules on. Three routes, the cost belonging to this item whichever is taken: retain the discarded candidates beside the converted entry, carry a separate overlap table, or convert only cells whose candidates span a single *containing* zone — the last is free but takes some of the 743 cells back out, so the count above is re-derived rather than reused. If the route is meant to preserve the candidates, add a test reading coverage independently of the index. Cheaper decided before the format is released than after.
