...
- **Status:** blocked on GEOM-3 — the precedence question is decided (above), but the planar ring through `h3.cell_to_boundary` vertices does not cover every point H3 assigns to the cell. A conservative full-cell predicate must be established before the converter can replace a candidate list with an unconditional zone. Found 2026-09-07 while measuring polygon overlap for GH-301; nothing has been implemented.
