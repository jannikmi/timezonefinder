...
- **It must not discard the candidates `timezones_at` reads.** That method walks an ambiguous cell's whole candidate list and reports every zone whose polygons contain the point — the dataset's overlaps are real, and `Asia/Urumqi` inside `Asia/Shanghai` is most of them. Collapsing a covered ambiguous cell into a unique-zone entry throws the other zones' candidates away, so exactly the cells this item converts are where that method would silently start answering one zone where it answers two; an under-reported list still reads as a plausible answer, which is why nothing about it is self-evident in review. **No fixture-wide test catches this**, and it is worth knowing why before trusting a green suite: `tests/test_overlapping_zones.py` compares the method against a full candidate scan, but that scan enumerates the *same* shortcut index, so a converted cell degrades both sides identically and they keep agreeing. What can catch it is that file's `DOCUMENTED_OVERLAPS` - six hard-coded coordinates, one per overlapping pair, asserted as exact lists - and `Asia/Urumqi` over `Asia/Shanghai` is both the first of them and one of the two pairs this conversion rules on. Treat those six as the regression surface, and add a test that reads coverage independently of the index if the converter takes a route that is meant to preserve the candidates. Three routes, and the cost belongs to this item whichever is taken: retain the discarded candidate list beside the converted entry, carry a separate overlap table, or convert only cells whose candidates span a single *containing* zone — the last is free but takes some of the 743 cells back out of the conversion, so the count above is re-derived rather than reused. Cheaper decided before the format is released than after.
...
- **Status:** blocked on GEOM-3 — the precedence question is decided (above), but the planar ring through `h3.cell_to_boundary` vertices does not cover every point H3 assigns to the cell. A conservative full-cell predicate must be established before the converter can replace a candidate list with an unconditional zone. Found 2026-09-07 while measuring polygon overlap for the candidate-ordering work; nothing has been implemented.
