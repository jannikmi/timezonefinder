...
- [Shortcut candidate ordering decisions](../../decisions/shortcut-candidate-ordering-decisions.md) — why *re-ordering* a cell's candidates is refused; this item removes the loop instead of re-ordering it, so that refusal does not cover it.
...
- `prototypes/shortcut_ordering_optimum.py` — its `areas` stage prints the coverage counts below; re-run it after a dataset or resolution change.
