...
- **Status:** blocked on GEOM-1 — the precedence question is decided (above), but the planar ring through `h3.cell_to_boundary` vertices does not cover every point H3 assigns to the cell. GEOM-1 must establish a conservative full-cell predicate before the converter can replace a candidate list with an unconditional zone. Found 2026-09-07 while measuring polygon overlap for the candidate-ordering work; nothing has been implemented.
