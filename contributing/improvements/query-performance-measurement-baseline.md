...
- **Freshness check**, before ranking anything on one of them:
  ...
  Read the [query-path change-classification log](query-path-change-classification-log.md) only
  when the freshness diff is non-empty or when adding a new classification.
