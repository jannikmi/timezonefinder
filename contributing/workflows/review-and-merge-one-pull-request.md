...
Park, but never let block: a pull request that needs a maintainer decision, a draft, one whose checks are red or absent, an improvement-pass pull request whose independent review never ran — a review that terminated on a usage limit is silence, not a pass; a manual pull request needs no such review, and neither does one the [testing rules](../development/testing-strategy-and-change-scope.md) exempt, which instead has to carry the sentence they require and is parked when it does not — one with unresolved review threads, and one from a fork whose head you cannot push to. Announce the pick with one clause per waiting pull request saying why it waits — the ordering is the cheapest thing for the maintainer to overrule.
