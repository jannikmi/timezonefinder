...
- Preserve the lookup fast path. A performance claim requires paired evidence from an instrument that can resolve the change — a removed count, a stage profile, or an in-query A/B when the batch suite cannot — with the observed spread and the acceleration backend named; an unresolved regression is reverted and recorded. A gain the suite cannot resolve is still a gain, but it is still a gain that has to be shown: the sub-floor rule relaxes the scope the improvement must be visible at, not the burden of proving it, so a change nothing measures as a win is a measurement item rather than a performance item. State the increment as what it is rather than as a suite-visible speed-up.
