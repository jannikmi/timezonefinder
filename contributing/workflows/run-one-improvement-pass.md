...
Before opening the pull request, take the independent review the [testing rules](../development/testing-strategy-and-change-scope.md) require - from a reader handed the diff and the routed rules but not this session's item, plan, or reasoning - and fix what it confirms. The automated round below is then spent on what an independent reader could not already see.

Opening the pull request is not the end of the pass either. Record its head commit and watch for the Codex review repository configuration starts. Inspect formal reviews, inline review comments, and reactions; `gh pr view --comments` alone misses inline findings. Record which commit a review covered, because a review older than the current head is evidence about that commit only. Silence is reported as silence, not treated as permission to spend another review.

Assess every finding against the current code rather than accepting it mechanically. Fix every confirmed problem the pull request introduced or exposed - a finding that would fail silently is never deferred to a follow-up item - strengthen the regression test where behavior changes, and run the narrow verification plus `make hook` before pushing. Answer each finding on its thread in one or two sentences - the commit that fixed it, or the evidence it is stale - then resolve it once settled. A genuine unrelated improvement becomes a new register item instead of silently broadening the pull request. If a confirmed finding cannot be fixed without violating a hard boundary or obtaining a maintainer decision, leave its thread open and the pull request unmerged, record the blocker, and report it rather than declaring the pass complete.
