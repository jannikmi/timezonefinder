...
Opening the pull request is not the end of the pass. Record its head commit and watch the configured Codex reviewer, `chatgpt-codex-connector[bot]`, until it has reviewed that head. Inspect formal reviews, inline pull-request review comments, and reactions on the pull request; `gh pr view --comments` alone misses inline findings. A Codex review whose commit is older than the current head is stale. A thumbs-up is a clean result only when it appeared after the current head was pushed and the head has not moved since; an older reaction is not evidence about a newer commit. Silence is not a clean review.

Assess every Codex finding against the current code rather than accepting it mechanically. Fix every confirmed problem introduced or exposed by the pull request, add or strengthen the regression test where behavior changes, and run the narrow verification plus `make hook` before committing and pushing the fixes. Reply briefly with concrete evidence when a comment is stale or incorrect. A genuine unrelated improvement becomes a new register item instead of silently broadening the pull request. If a confirmed finding cannot be fixed without violating a hard boundary or obtaining a maintainer decision, leave the pull request unmerged, record the blocker through the normal register rules, and report it rather than declaring the pass complete.

After pushing review fixes, comment `@codex review` on the pull request and watch again. Repeat until Codex reports no actionable finding on the current head. A review fix invalidates the earlier final gate: once the review is clean, fetch and rebase if `origin/master` moved, then repeat the full final gate. Any rebase or corrective push creates a new head that needs another Codex review. The pass is complete only when the same head is current with `origin/master`, passes the required gate, and has a clean Codex disposition.
