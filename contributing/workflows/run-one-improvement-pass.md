...
- Preserve the lookup fast path. A performance claim requires paired evidence from an instrument that can resolve the change — a removed count, a stage profile, or an in-query A/B when the batch suite cannot — with the observed spread and the acceleration backend named; an unresolved regression is reverted and recorded. A gain the suite cannot resolve is still a gain: take the item on a clear benefit, and state the increment as what it is rather than as a suite-visible speed-up.
