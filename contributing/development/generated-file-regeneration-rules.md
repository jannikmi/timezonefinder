...
- `scripts/measure_tzfpy_agreement.py` writes `docs/tzfpy_agreement.json` through the shared JSON writer and renders `docs/tzfpy_agreement_by_distance.svg` with deterministic matplotlib settings and exactly one final newline. The JSON is the measured run and the SVG is that run drawn; `tests/test_tzfpy_agreement.py` holds them together byte for byte, in the chart test environment that carries the pinned plotting stack
