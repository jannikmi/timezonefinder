...
The second is specific to the Numba run: **installing Numba changes more than the point-in-polygon kernel**. ``utils.validate_coordinates`` calls two ``njit``-compiled scalar helpers when Numba is importable and two plain comparisons when it is not - 299 ns against 241 ns - and every query pays it on the way in, before any geometry. So a C-extension lookup measured beside Numba is not quite the C-extension lookup a plain install runs, and the effect is largest on the unique-shortcut rows, where validation is most of the query and no polygon is ever tested.
