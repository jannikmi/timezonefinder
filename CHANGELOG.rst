...
* the improvement register's ranking table now holds only items a pass can still take. Rejected, withdrawn and out-of-scope entries keep their entry - the argument against them is what stops the next pass re-proposing them - but their row moves to a ``Closed`` table, since the ranking orders work and a closed item has none. ``blocked``, ``parked`` and ``conditional`` rows stay, because those resolve without the entry changing. ``tests/test_improvement_ledger.py`` asserts the placement in both directions; without it the rule was unenforceable, as both tables sit under one heading and the existing one-row-per-entry assertion is satisfied wherever the row is. The two skills that maintain the file record the convention as part of closing an entry
