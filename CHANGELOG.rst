...
* the code-release data-dependency guard accepts all platform wheels for one build, but refuses a staging directory containing wheels from more than one ``timezonefinder`` version. A stale wheel can no longer make the release check validate metadata from a version other than the one about to be published
