...
* ``docs/benchmarking_methodology.rst`` gains two sections. One covers comparing two candidate implementations of a single stage, which the existing suites do not do and which three different measurement designs got wrong in a row - each of them flattering the newer candidate, by up to 13 percentage points. The other explains reading a stage's share of a query asymmetrically: it bounds what optimising that stage can win and says nothing about what redesigning it can lose, which for the shortcut lookup is at most ~2.7% of a mixed workload against a measured +29% for one plausible redesign. ``potential-improvements.md`` records the tooling gap the same measurements exposed: the pull request benchmark comparison measures base and head in two sequential runs reduced to a single estimator each, so it cannot resolve the low-single-digit effects that three open performance items will need to demonstrate.

* a new prototype, ``prototypes/shortcut_file_format_bench.py``, settles which data structure the H3 shortcut index should use, and whether replacing the packaged binary earns the data format version bump and ordered two-distribution release it would cost. It writes three candidate structures, reads each back, and checks all 41,162 cells resolve identically before timing anything. On query speed - the reason the index exists - the best candidate and the current implementation are indistinguishable, within a couple of percent across four coordinate strata, so the change has to be justified on its other axes: 12x less memory, 600x faster construction and a 3x smaller packaged file. Among the flat candidates one storing each distinct entry once wins, since only 6.9% of entries are distinct - the ocean zones repeat a single zone id across thousands of cells. Most of the construction win is available without any format change, so the conclusion is to sequence that cheaper fix first. The prototype also settles how entries should be addressed: by arithmetic on H3's index bits, which is 3.3x smaller and 12% faster to load than storing the cell ids, and which is made safe by a build-time check that the arithmetic agrees with H3's public accessors on every cell rather than by avoiding the coupling. A third of that index is structurally unreachable - the arithmetic is base-8 per digit where H3 digits only take 0-6 - and since which slots are live follows from the resolution rather than the data, the file stores only the reachable ones and the reader expands them, taking the packaged shortcut binary to about an eleventh of its current size.

* a new prototype, ``prototypes/shortcut_layout_bench.py``, prices the flat shortcut layouts proposed in issue #477 on both the scalar and the batched lookup path, by swapping three candidate layouts underneath a real ``timezone_at`` and through a prototype N-point resolver rather than timing lookups in isolation. It settles which layout is worth building - the sorted-key ``np.searchsorted`` variant roughly doubles a unique-zone query and, vectorised over ten thousand points, is *still* slower than a Python loop of dict lookups, while the direct-index variant costs nothing measurable on the scalar path, is thirty times faster batched and is the smallest of the three - and shows its bit slicing to be a bijection over the whole H3 resolution rather than a hash that happens to fit the current keys. Its unlooked-for finding is that a batched lookup is bounded by ``h3-py`` exposing no vectorised ``latlng_to_cell``, not by the shortcut structure. ``potential-improvements.md`` and issue #499 record the consequence: the batch API is no longer blocked on this layout.
...
* ``timezonefinder.flatbuf.schemas.__all__`` no longer declares three names the module does not define. It listed ``polygons``, ``hybrid_shortcuts_uint8`` and ``hybrid_shortcuts_uint16`` - the ``.fbs`` schema *files* sitting next to the module, not submodules of it - so ``from timezonefinder.flatbuf.schemas import *`` raised ``AttributeError``. ``tests/test_documented_contracts.py`` now checks every ``__all__`` in the package against the attributes its module actually has, which nothing did before
