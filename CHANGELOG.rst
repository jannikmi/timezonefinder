...
* the packaged ``coordinates.fbs`` files now carry a FlatBuffers file identifier and a layout version recording how the coordinates are encoded and which polygons the file holds, and ``TimezoneFinder`` raises a ``ValueError`` naming the offending file when it does not match - previously such a directory was read without complaint and produced wrong timezones. If you compile your own data and point ``bin_file_location`` at it, regenerate it once with ``scripts/file_converter.py``, since both the coordinate layout and the hole storage changed in this release. The marker tracks what the file holds rather than the package version, so this is not a per-release obligation
...
* the command line script gained a ``--stdin`` streaming mode for annotating a file of coordinates in one command: it reads delimited rows from standard input and writes each row back out with a ``timezone`` column appended, building the finder once instead of paying full initialisation per coordinate. The coordinate columns are found in the header row by name (``lng``/``lon``/``long``/``longitude``/``x`` and ``lat``/``latitude``/``y``, case-insensitive), so their position in the file does not matter, and ``--lng-col``/``--lat-col`` name them explicitly - by header name or 1-based column number - for other headers or for input without one. The order is never guessed: headerless input without those flags is rejected, because a swapped pair is still a valid coordinate for any longitude between -90 and 90 and would resolve to a real but wrong timezone rather than failing. ``-d``/``--delimiter`` sets the field delimiter for input and output (default ``,``, tab as ``'\t'``), and standard CSV quoting is preserved both ways. ``-f`` applies to the whole stream and ``-v`` is rejected, since verbose output is per query. There is always exactly one output row per input row, so a caller can consume the two in step: a row that is blank, too short, non-numeric or outside the valid coordinate range comes back with an empty ``timezone`` cell plus a warning on stderr giving the row number and the reason, and never ends the stream. An empty cell is also what a genuine "no timezone here" looks like - what ``timezone_at_land()`` returns for every ocean point - so the two are distinguished by the exit code, which is ``1`` if any row was rejected. The new ``--in-memory`` flag reads the polygon coordinate data into RAM rather than memory-mapping it, trading an order of magnitude more memory for measurably faster lookups, which only a long stream can amortise; it does not apply to ``-f 3``/``-f 4``, which load no polygon data. Solves issue #504
...
* added a ``cut-release`` coding-agent skill (``.claude/skills/``) driving the manual release end to end, in two halves split by the maintainer's merge: it checks the ``X.X.X (unreleased)`` section is release-ready and complete against every commit since the last tag, applies the changelog rules that make a released section read as one step, then proposes ``patch``/``minor``/``major`` as three concrete version numbers with the bullets behind each and **stops for the maintainer's decision** - and only afterwards lands the bump as a release PR and, once that is merged, pushes the tag. Both irreversible steps are the maintainer's and are asked separately, since the level is a promise about the public API and the tag is the publish, which PyPI will not let a version take back. It records what the pipeline makes non-obvious: the tag-push run re-reads ``pyproject.toml`` at the tagged commit, so the tag and the version must agree; the release commit is the bump and the changelog and nothing else, so ``make reports`` must not run in it; and the tag has to be pushed promptly after the merge, because the master push run's own ``release`` job creates the GitHub Release and with it the tag, after which a ``git push`` of that tag is a no-op that fires no webhook - which is why the automated data path tags immediately too. ``CLAUDE.md``'s claim that regenerating data warrants a minor bump is corrected along the way: ``update_data.sh`` bumps ``patch`` and the last four data releases were patches
