...
* the packaged ``coordinates.fbs`` files now carry a FlatBuffers file identifier and a layout version, and ``TimezoneFinder`` raises a ``ValueError`` naming the offending file when it is asked to load coordinate data written by an incompatible version. This matters if you compile your own data and point ``bin_file_location`` at it: such a directory must be regenerated with ``scripts/file_converter.py`` from the matching checkout. Previously a stale directory was read without complaint and produced wrong timezones
