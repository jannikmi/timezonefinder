...
* the tracked estimator is pytest-benchmark's ``min``, not its ``mean``: every round performs the same fixed batch of work, so the fastest round is least perturbed by the shared, virtualised runners. ``scripts/normalize_benchmark_json.py`` rewrites ``ops``/``mean`` accordingly, since the action's pytest extractor reads only ``stats.ops``. The alert threshold is measured rather than guessed - ``scripts/benchmark_noise.py`` / ``make benchmark-noise`` repeat the CI measurement on unchanged code; 5 runs on 5 separate ``ubuntu-latest`` runners spread at most 106.8%, giving a 110% threshold, and alerting ships non-blocking (``fail-on-alert: false``) until the trend chart confirms it does not fire on unchanged code. ``make benchmarks-ci`` reproduces the CI measurement locally; ``CONTRIBUTING.md`` documents how to read the PR comment and that local numbers are not comparable to CI numbers
...
* guarded the benchmark plumbing against silent drift: benchmark node ids are pinned by ``tests/test_benchmark_names.py`` so a rename fails loudly instead of resetting the trend history; ``tests/test_benchmark_workflows.py`` asserts that the constants duplicated across the two workflows (``BENCHMARK_SUITE_NAME``, ``ALERT_THRESHOLD``, ``REPORT_FILENAME``, trend data location, artifact name) actually agree, where a one-sided edit previously had no failure mode at all and would compare against nothing; and ``docs/benchmark_results_*.rst`` carry a "Benchmark Input Provenance" stamp naming the fixture and timezone data versions the timings were measured against, so a report can no longer describe a workload that no longer exists
