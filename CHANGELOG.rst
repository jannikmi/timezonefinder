...
* a new prototype, ``prototypes/shortcut_file_format_bench.py``, settles which data structure the H3 shortcut index should use, and whether replacing the packaged binary earns the data format version bump and ordered two-distribution release it would cost. It writes three candidate structures, reads each back, and checks all 41,162 cells resolve identically before timing anything. Replacing the decoded dictionary with a flat file is worth 12x the memory, 600x the load time and 3x the file size; among the flat candidates a structure storing each distinct entry once wins, since only 6.9% of entries are distinct - the ocean zones repeat a single zone id across thousands of cells - and sharing costs nothing at lookup. The lookup is unchanged inside a real query. Most of the load win is available without any format change, so the conclusion is to sequence that cheaper fix first, and the refused-options block records the alternatives that lost and why.
