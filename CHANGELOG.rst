...
* migrated benchmarking from the hand-rolled, one-shot ``timeit``-based timing in ``scripts/check_speed_*.py`` (no warmup, no repeats, mean-only) to ``pytest-benchmark`` suites under the new ``benchmarks/`` package (``test_timezone_finding.py``, ``test_inside_polygon.py``, ``test_initialization.py``), which calibrates warmup/rounds and reports full min/max/mean/median/stddev/IQR statistics; excluded from ``make test``/``make testall`` via ``testpaths`` and run explicitly via ``make speedtest`` (quick, tracked core subset) or ``make benchmarks``/``make reports`` (full suite, JSON output, regenerated ``docs/benchmark_results_*.rst``). Measurement (pytest-benchmark) and rendering (new ``scripts/render_benchmark_reports.py``, reusing the existing ``BenchmarkReporter``/``add_system_status_section``) are now fully decoupled, so the docs can be regenerated from a stored JSON without re-running anything; the numba/clang flags recorded in that JSON describe the machine that produced it, not whichever machine later renders it. Benchmark node ids are pinned by ``tests/test_benchmark_names.py`` so a rename fails loudly instead of silently resetting a future trend chart's history (plan 03). Never combine a benchmark run with ``pytest-run-parallel``'s ``--parallel-threads`` - see ``CONTRIBUTING.md``
* ``update_data.sh`` now runs ``make reports`` automatically after regenerating the boundary data, ``DATA_VERSION``, and the benchmark fixtures, so ``docs/benchmark_results_*.rst`` (and ``docs/data_report.rst``) no longer go stale relative to the binary data an update PR actually ships - previously they had to be refreshed by hand and easily got forgotten. ``scripts/file_converter.py``'s ``parse_data()`` still does not trigger this by itself (the pytest-benchmark suite needs fixtures already back in sync with ``DATA_VERSION``, which only ``update_data.sh`` guarantees), so a standalone ``make parse``/``make testparse`` still needs a manual ``make reports`` afterwards
...
* added continuous benchmarking on CI (solves issue #150), deliberately kept out of the release/publish pipeline in ``build.yml``: the new ``.github/workflows/benchmark.yml`` runs the tracked core benchmark subset on every pull request and on every push to ``master``, records the result via ``benchmark-action/github-action-benchmark`` and publishes a long-term trend chart to the ``gh-pages`` branch. The pull request job holds no write permissions and no secrets, so it behaves identically for branch and fork PRs and a fork PR can never fail for want of a token; the comparison comment is posted by the separate, privileged ``.github/workflows/benchmark-comment.yml`` via ``workflow_run`` (``pull_request_target`` was rejected: it would grant write permissions in the same context as untrusted PR code, which is not a trade a package published to PyPI should make). CI tracks the no-numba/clang C extension configuration - what a plain ``pip install timezonefinder`` gives you - and asserts the active point-in-polygon acceleration path (new ``scripts/assert_acceleration_path.py``) rather than assuming it, since a silent numba install would corrupt the entire trend history
* the value tracked on the trend chart is pytest-benchmark's ``min``, not its ``mean``: every benchmark round performs the same fixed batch of work, so the fastest round is the estimator least perturbed by the shared, virtualised CI runners. ``benchmark-action/github-action-benchmark``'s pytest extractor reads only ``stats.ops`` (``= 1 / stats.mean``), so the new ``scripts/normalize_benchmark_json.py`` rewrites ``ops``/``mean`` from the chosen estimator before the report is handed over - leaving an ordinary pytest-benchmark JSON that the action still parses natively, rather than hand-rolling storage or comparison
* added ``scripts/benchmark_noise.py`` and ``make benchmark-noise``, which repeat the exact CI measurement on unchanged code and report the observed per-benchmark spread plus an alert threshold derived from it. GitHub-hosted runners vary substantially between runs of the same job, so the alert threshold comes from a measurement rather than from the action's default: five runs of identical code on five separate ``ubuntu-latest`` runners spread 119.6% (CV 6.0%) on the tracked ``min``, giving a threshold of 130% (worst observed spread plus 20% headroom). Alerting nevertheless ships non-blocking (``fail-on-alert: false``) until the trend chart confirms the gate does not fire on unchanged code over time. ``make benchmarks-ci`` reproduces the CI measurement locally; ``CONTRIBUTING.md`` documents how to read the PR comment, what to do when an alert fires, and that local numbers are not comparable to CI numbers
* the CI-tracked benchmark set is chosen to be globally representative rather than worst-case: ``test_timezone_at[random-in_memory]`` (uniformly random query points, containing unique- and ambiguous-shortcut queries in their real ~25.5% ratio) is the headline, so a speedup is weighted by how much of a realistic query load it actually helps - previously only the ambiguous-shortcut case was tracked, which weighted the most expensive path as if it were all of the traffic and made unique-shortcut improvements invisible. ``unique_shortcut-in_memory`` and ``ambiguous_shortcut-in_memory`` are tracked alongside it as diagnostics: on the clang configuration CI tracks, an ambiguous lookup costs ~14x a unique one (~7x with numba) and therefore takes ~83% of the wall clock despite being ~25% of the queries, so a unique-path win moves the headline by only ~0.17x its true size and needs an undiluted metric to be attributable
* benchmark fixture query points are now drawn uniformly per unit of **surface area** (new ``get_rnd_query_pt_area_weighted``) instead of uniformly in latitude, which oversampled the poles ~2.5x: 33.3% of the old random points lay beyond 60 degrees latitude (either hemisphere) against 13.4% for an area-uniform sample, inflating the ambiguous-shortcut share to 30.5% where the shortcut index's own ``unique surface fraction`` implies 25.5%. The regenerated fixtures measure 13.2% and 26.0%. ``tests/auxiliaries.py``'s ``get_rnd_query_pt`` is deliberately unchanged - correctness and fuzz tests benefit from the polar edge cases, benchmarks must represent real query load
* ``docs/benchmark_results_*.rst`` now carry a "Benchmark Input Provenance" stamp naming the fixture version and timezone data version the timings were measured against, recorded into the benchmark JSON by ``benchmarks/conftest.py``'s ``pytest_benchmark_update_machine_info`` hook (alongside the existing numba/clang flags and batch size) and read back at render time. Regenerating the fixtures or updating the boundary data previously left the committed reports describing a workload that no longer existed, with nothing in the file to say so. The values are read through the fixture loader's validation, so a fixture set out of sync with the checkout fails the benchmark run rather than being stamped into a report, and a JSON predating the fields now fails to render instead of silently producing an unstamped one
* the benchmark fixture loader now rejects fixtures whose generation logic no longer matches the checkout, via a ``FIXTURE_VERSION`` (moved to ``tests/auxiliaries.py``, since the generator imports from there) and the ``point_sampler`` recorded in ``metadata.json``. Previously ``FIXTURE_VERSION`` was written and never read, and the loader validated only ``DEBUG`` and ``DATA_VERSION`` - so new generator code with stale committed ``.npy`` files loaded cleanly and benchmarked a workload nobody had described. A distribution test on the committed random points also fails if the pole-biased sampler is ever reinstated
* raised ``BATCH_SIZE`` from 1,000 to 2,500. The unique-shortcut benchmark is ~7x cheaper than the ambiguous one and therefore the noisiest in relative terms, and since one alert threshold covers every tracked benchmark, the noisiest sets it; more rounds cannot help because the tracked estimator is the ``min``, so per-round resolution is the lever. Documented the previously implicit ceilings (5,000 per points fixture, 3,333 per point-in-polygon stratum) that ``BATCH_SIZE`` must stay under
* the CI alert threshold tightened from 130% to 110%, re-derived against the new three-benchmark set: 5 runs of identical code on 5 separate runners now spread at most 106.8% (random 106.8% / CV 2.7%, unique 105.0% / 1.6%, ambiguous 104.9% / 1.7%), where the previous single-benchmark measurement spread 119.6% (CV 6.0%). The larger batch is what bought the resolution - the shortest benchmark turned out to be the *least* noisy of the three, not the most. Alerting still ships non-blocking (``fail-on-alert: false``)
* added ``tests/test_benchmark_workflows.py``, which asserts that the constants the two benchmark workflows must agree on actually do: GitHub Actions workflows cannot import constants from each other, so ``BENCHMARK_SUITE_NAME``, ``ALERT_THRESHOLD`` and ``REPORT_FILENAME`` are duplicated literally in ``.github/workflows/benchmark.yml`` and ``.github/workflows/benchmark-comment.yml``, previously held together by nothing but a "must match" comment. A one-sided edit had no failure mode at all - the comparison job would look up a suite name or a filename that no longer corresponds to what the measuring job produced and silently compare against nothing. The test also covers the trend data location (``gh-pages-branch``, ``benchmark-data-dir-path``) and checks that the artifact both consumers download is the one the measure job uploads
