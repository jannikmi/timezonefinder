...
* the latitude block index is checked where it is produced and over what the repository ships, by ``scripts.data_integrity.validate_block_index``, which re-derives every ring's block ranges from the stored coordinates. Nothing about a wrong range announces itself at lookup time - the kernels skip on it, so a range one unit too narrow drops a real crossing and answers one point differently - and re-deriving it in every user's process is exactly what this repository's integrity checks exist not to do
...
* ``get_last_change_idx`` moves out of the runtime modules to sit beside the shortcut index builder, its only caller. Precomputing the value into the shortcut binary removed the last per-query call to it, but the ``@njit`` function stayed in ``timezonefinder/utils_numba.py``, where numba compiles and binds it at import in every process for something no lookup runs. It is now plain Python next to the builder that writes those values, and ``scripts/data_integrity.py`` re-checks the committed values against that same implementation, so the two still cannot drift
...
* the offset table the lookup path addresses polygons through is checked against the FlatBuffers reader for every polygon in both coordinate files, by ``scripts.data_integrity.validate_coordinate_offset_table`` - run by the converter over the files it just wrote and by the test suite over the files the repository ships, like the hole reference checks next to it. The fast path assumes what the reader guarantees: that a table sits where its reference says, that vtables are aligned, that the coords field is present. Nothing about a violated assumption announces itself, since an offset landing one word early still yields plausible ``int32`` coordinates and therefore a plausible wrong timezone - so it is established exhaustively where the data is produced rather than defended cheaply where it is read
