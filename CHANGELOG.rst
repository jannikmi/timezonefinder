...
* the packaged ``coordinates.fbs`` files now carry a FlatBuffers file identifier and a layout version recording how the coordinates are encoded and which polygons the file holds, and ``TimezoneFinder`` raises a ``ValueError`` naming the offending file when it cannot read that layout - previously such a directory was read without complaint and produced wrong timezones. Older layouts stay readable where they can be read correctly, so a ``bin_file_location`` directory compiled by an earlier release does not have to be regenerated; the reverse direction is what the marker protects, since a released version handed newer data would otherwise return plausible wrong answers. The marker tracks what the file holds rather than the package version, so bumping it is not a per-release obligation
...
* the data converter also asserts that hole deduplication keeps paying off: it fails if fewer than 90% of holes match a boundary polygon (96.4% in the current data), because a future upstream release that stopped emitting enclaves as shared rings would still compile and still return correct timezones, just with the packaged data quietly re-inflated. That floor is a statement about the upstream dataset rather than about file consistency, so it is a separate check - a small custom region can legitimately have few enclaves while being perfectly well formed. ``prototypes/hole_boundary_redundancy.py`` is the study behind both the threshold and the decision to keep the unmatched holes stored inline: it reads the upstream GeoJSON, so re-running it against a new release re-verifies the assumption rather than restating it
