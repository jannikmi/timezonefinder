...
* the command line script gained a ``--stdin`` streaming mode: it reads ``lng,lat`` pairs from standard input, one per line, and writes one result per line, building the finder once instead of paying full initialisation per coordinate. ``-f`` selects the lookup function for the whole stream and ``-v`` is rejected, since verbose output is per query. There is always exactly one output line per input line, so a caller can consume the two in step: a line that is blank, is not two numbers, or names a coordinate outside the valid range produces an empty output line plus a warning on stderr giving the line number and the reason, and never ends the stream. An empty output line is also what a genuine "no timezone here" looks like - what ``timezone_at_land()`` returns for every ocean point - so the two are distinguished by the exit code, which is ``1`` if any input line was rejected. The new ``--in-memory`` flag reads the coordinate data into RAM rather than memory-mapping it, roughly 1.3x faster per lookup for tens of MB more, which only a long stream can amortise. Solves issue #504
