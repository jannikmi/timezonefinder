...
* ``docs/benchmarking_methodology.rst`` gains two sections. One covers comparing two candidate implementations of a single stage, which the existing suites do not do and which three different measurement designs got wrong in a row - each of them flattering the newer candidate, by up to 13 percentage points. The other explains reading a stage's share of a query asymmetrically: it bounds what optimising that stage can win and says nothing about what redesigning it can lose, which for the shortcut lookup is at most a few percent of a mixed workload against a measured +29% for one plausible redesign. ``potential-improvements.md`` records the tooling gap the same measurements exposed: the pull request benchmark comparison measures base and head in two sequential runs reduced to a single estimator each, so it cannot resolve the low-single-digit effects that three open performance items will need to demonstrate.
...
* the ``code-quality-pass`` and ``roadmap-pass`` coding-agent skills are one ``improvement-pass`` skill, and ``potential-improvements.md`` is now the single register behind it - one ranking over every improvement worth making, whatever its area. The old pair sorted work into two kinds and let *the skill* decide whether to consult the maintainer; what actually decides that is a property of the individual change - whether it alters observable behaviour, and whether it carries a choice that outlives the pass and is expensive to reverse - so the merged skill asks those two questions per item and enumerates no categories at all. Nothing is out of scope for being the wrong kind of finding: a correctness defect, a slow path, an awkward API, a docs page that lies, a release step that can fail silently and a name that misleads are ranked in one list, because there is one reviewer's attention to spend on them and sorting first is how the cheap items get taken because they are cheap. One pass now takes exactly one item, the highest-ranked eligible one (unclaimed, preconditions met, its maintainer-owned decisions already recorded, small enough to review), and ends in one pull request; the former ~400-line diff budget is gone rather than reinterpreted, since a line count decides nothing a reviewer cares about - a slice is sized by whether it carries one story that can be described by a single changelog sentence and reviewed in one sitting - and an item awaiting a decision no longer stalls the ones below it. The register absorbed the roadmap issue that used to hold the ranking, the sequencing rules and the recorded decisions, which closes it: reasoning kept outside the repository goes stale silently, because nothing references it, no check reads it and a reviewer never sees it in a diff - inside, every change to the ranking arrives as a reviewable diff and the decisions that refuse a re-proposal sit next to the entries they constrain. Issues remain where a single item is worked out. ``tests/test_improvement_ledger.py`` asserts that the ranking table and the entries name the same items, and that no entry carries a status meaning the work is done - an entry whose work landed is deleted, row and all, since one left behind reads exactly like an open one and the next pass pays full price to rediscover that there is nothing to do. Neither invariant shows up in a diff review, because both halves read as correct on their own. Performance items are ranked on a measured share rather than on an intuition about what looks slow: the register carries the baseline ``prototypes/query_stage_profile.py`` produced - the per-query denominators, the commit they were taken at and a one-command freshness check - benefit is stated as the ceiling a change can remove and refused as a speed-up when that ceiling sits under the machine's own run-to-run noise, and the pull request that moves the critical path re-measures and updates every share it moved, since a share that has gone stale reads exactly like a current one. Because those figures come off one development machine, the register also says which of them survive leaving it: hit counts do, being a property of the code rather than the hardware; shares within a query travel only as orders of magnitude, since the stages are bound by different things; absolute nanoseconds do not travel at all. Benefits are therefore stated as the count a change removes and as a share of a realistic mixed workload rather than of one stratum, read from the ``clang``/memory-mapped configuration a plain install actually runs. The prototype's own ``FINDINGS`` block states the same three tiers, so the caveat travels with the numbers rather than only with the register that quotes them
* the ``improvement-pass`` coding-agent skill no longer stops to ask anything, and the choices it may not make itself go to a new ``maintainer-decisions`` skill (``.claude/skills/``). The split is by whether the maintainer is at the keyboard, which is what the single skill could not express: a pass runs unattended, so a design question put to them mid-run reached nobody, the item stalled and everything ranked below it waited with it - and an answer that did arrive was consumed in the same chat session and written down nowhere the pass that finally implements the item could read it. A pass now records the question and takes the next eligible item instead: ``Status: needs ...`` plus a ``**Decision needed:**`` bullet carrying the question, the options with their trade-offs and a recommendation, written while the code it concerns has just been read. ``tests/test_improvement_ledger.py`` enforces that pairing in both directions - a ``needs`` status without the bullet says an entry is waiting without saying what for, and the bullet without the status is a question no search over statuses will ever surface. The new skill takes it from there, in one round: it re-verifies each question against the current code, since an entry's premise goes stale faster than its location and a question can turn out to be already answered or to be a different question; prices whatever a single command can price rather than speculating; briefs at most four, which is the point at which a round stops being read; and takes them one at a time - the brief in full in chat, then a discussion for as long as the maintainer wants one, then the decision - because the evidence never fits in a question dialog, and a round collected as a single form asks for every answer at once from somebody who has just read every brief at once, leaving no point at which they can put another option on the table or disprove a premise while either still matters. An option the maintainer raises is priced like the others rather than talked down, and a recommendation the discussion undermines is withdrawn rather than defended. Every answer is recorded in ``potential-improvements.md`` with its rationale and the options it refused, and pushed as it is given rather than at the end of the round. It never implements what it asked about, so a design is reviewed as a decision rather than arriving fused to its implementation with nobody left to catch it, and a question that goes unanswered still leaves its brief in the entry for the next round. The four entries that were already waiting carry theirs: whether the memory-mapped accessor may hold a whole-file view for its lifetime, what a tripped data-update guardrail should do, which invariants ``validate-data`` checks first, and whether an ``update-data`` subcommand is still wanted now that the dataset is pip-installable
