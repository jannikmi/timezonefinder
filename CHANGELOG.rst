...
* polygon coordinates are now stored one axis at a time in the packaged ``coordinates.fbs`` files - all x values followed by all y values per polygon, instead of interleaved ``x0, y0, x1, y1, ...``. The raycasting point in polygon test scans a single axis per iteration, so contiguous per-axis blocks halve the cache lines it touches and let both acceleration backends read an axis without copying: point-in-polygon is ~1.6x faster on a median polygon and ~2.5x faster on the largest ones via the C extension, and 14-25% faster via Numba. The bundled data was regenerated accordingly
* the packaged ``coordinates.fbs`` files now carry a FlatBuffers file identifier and a layout version recording the coordinate encoding, and ``TimezoneFinder`` raises a ``ValueError`` naming the offending file when the encoding does not match the one it reads. Previously such a directory was read without complaint and produced wrong timezones. This matters if you compile your own data and point ``bin_file_location`` at it: because the coordinate layout changed in this release, those directories have to be regenerated once with ``scripts/file_converter.py``. The marker tracks the encoding rather than the package version, so this is not a per-release obligation - compiled data stays valid across upgrades that leave the format alone
* the memory footprint of every finder configuration is now measured and published: the new `memory report <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_memory.html>`__ states what ``TimezoneFinder``, ``TimezoneFinder(in_memory=True)`` and ``TimezoneFinderL`` each cost, separating what they allocate (``tracemalloc``) from what they make resident (RSS, which additionally counts memory-mapped pages). The distinction is the point: the default mode maps the coordinate data instead of reading it, so its allocation stays an order of magnitude below the in-memory mode's while its resident set grows only with the pages a lookup actually touches. This replaces the previous documentation, which claimed a process ceiling of 40MB and a 41MB data directory - both long out of date - and fills the ``Memory Usage`` row of the package comparison, which read ``TBA``
* ``README.rst`` and the documentation landing page now open with a project banner, and their badge block is corrected: the ``code style: black`` badge named a formatter this project has never used and is replaced by ``ruff``, and a supported-Python-versions badge was added. The banner is referenced by absolute URL in ``README.rst``, since PyPI serves the long description without the repository and a ``docs/…`` path renders as a broken image there
* rewrote the `package comparison <https://timezonefinder.readthedocs.io/en/latest/alternatives.html>`__ page. It now states its position in prose before the first table - border correctness is what this package optimises for, speed is the constraint that work happens under - and says plainly when ``tzfpy`` is the better choice. Every quantitative cell names what it measures and links its source, the ``Distribution Size`` figure is corrected (it predated the return to the full dataset and was roughly half the current wheel), and the speed row carries an explicit caveat that the two numbers come from different machines and workloads and are indicative rather than a like-for-like measurement. The decision table drops the rows on which the two packages do not differ
* two new documentation pages: `Architecture <https://timezonefinder.readthedocs.io/en/latest/architecture.html>`__ describes the lookup pipeline, the three point-in-polygon backends and the memory modes, and states the ceilings this package deliberately does not exceed - unsimplified geometry, ~1 cm coordinate resolution, no general-purpose spatial code. `Benchmarking Methodology <https://timezonefinder.readthedocs.io/en/latest/benchmarking_methodology.html>`__ documents how the published numbers are produced and what they can and cannot tell you: ``ubuntu-latest`` pins the runner image and not the CPU, which is why a pull request is measured against its own merge base on the same runner and why every alert threshold is derived from measured noise. It was previously addressed only to contributors, in the second half of ``CONTRIBUTING.md``, which now keeps the operational instructions and links to it
* the H3 resolution choice in the `data format documentation <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ is no longer asserted to "offer a good balance" but reports the study behind it (``prototypes/single_resolution_bench.py``): the hybrid index is ~1.6 MB at resolution 3, while resolution 4 would exceed 10 % of the packaged polygon data for gains that do not justify it
...
* replaced the hand-rolled ``timeit`` timing in ``scripts/check_speed_*.py`` with ``pytest-benchmark`` suites under ``benchmarks/``, which calibrate warmup/rounds and report full min/max/mean/median/stddev/IQR. Run via ``make speedtest`` (tracked core subset) or ``make benchmarks``/``make reports`` (full suite, JSON output, regenerated ``docs/benchmark_results_*.rst``); excluded from ``make test``/``make testall`` via ``testpaths``. Measurement and rendering are decoupled, so the docs can be regenerated from a stored JSON without re-running anything. Never combine a benchmark run with ``pytest-run-parallel``'s ``--parallel-threads`` - see ``CONTRIBUTING.md``
* benchmark inputs are deterministic committed fixtures (``tests/fixtures/benchmarks/``, generated via ``scripts/generate_benchmark_fixtures.py`` / ``make benchmark-fixtures``) instead of freshly drawn random data, so two runs of the same commit execute the exact same workload. Query points are sampled uniformly per unit of surface area rather than uniformly in latitude, which oversampled the poles ~2.5x and inflated the ambiguous-shortcut share; the point-in-polygon fixture is stratified by polygon vertex count so the largest polygons are not hidden behind an unweighted average. ``tests/auxiliaries.py``'s ``get_rnd_query_pt`` stays pole-biased on purpose - correctness and fuzz tests want the polar edge cases. The loader rejects fixtures that no longer match the checkout (``DATA_VERSION``, ``FIXTURE_VERSION``, recorded ``point_sampler``), and ``update_data.sh`` regenerates them right after bumping ``DATA_VERSION``
* added continuous benchmarking on CI (solves issue #150), deliberately kept out of the release/publish pipeline in ``build.yml``: ``.github/workflows/benchmark.yml`` runs the tracked core subset on every pull request and every push to ``master``, records the result via ``benchmark-action/github-action-benchmark`` and publishes a `trend chart <https://jannikmi.github.io/timezonefinder/dev/bench/>`__ to ``gh-pages``, now linked from the README and the performance docs (the action writes only ``dev/bench``, so the Pages root - a 404 until now - redirects there). The pull request job holds no write permissions and no secrets, so branch and fork PRs behave identically; the comparison comment is posted by the separate, privileged ``.github/workflows/benchmark-comment.yml`` via ``workflow_run`` (``pull_request_target`` would grant write permissions in the same context as untrusted PR code). CI tracks the no-numba/clang C extension configuration - what a plain ``pip install timezonefinder`` gives you - and asserts the active acceleration path (``scripts/assert_acceleration_path.py``), since a silent numba install would corrupt the whole trend history
* a pull request is benchmarked against **its own merge base, measured in the same job on the same runner**, instead of against the stored ``master`` baseline. ``runs-on: ubuntu-latest`` pins the runner *image*, not the CPU: the pool serves AMD EPYC 9V74, AMD EPYC 7763 and Intel Xeon Platinum 8573C parts at 2.30-3.69 GHz, which spread the tracked ``min`` by 134-158% across eleven runs of an **unchanged lookup path** - enough that a merged 1.5x improvement once showed up on the trend chart as a 21% regression. ``scripts/compare_benchmark_runs.py`` renders the base/head ratio table, verifying rather than assuming that both sides ran on one machine and flagging a differing batch size, fixture set, boundary data or acceleration path. The base is measured twice, sandwiching the head, so a runner drifting mid-job shows up in the base's own spread; this roughly doubles the measuring job (~2-3 min, almost all of it the second checkout, ``uv sync`` and C extension build). Reporting only for now - the same-runner residual has not been measured in isolation, and ``--fail-on-regression`` stays off until it has
* every benchmark run now says which machine produced it: ``scripts/describe_benchmark_machine.py`` prints the CPU, clock, acceleration path and workload provenance to the job summary, and ``scripts/normalize_benchmark_json.py`` stamps the same label into ``stats.rounds`` - the one field ``benchmark-action``'s pytest extractor passes through verbatim - so the trend chart's tooltip attributes a data point to a CPU long after its artifact has expired
* the tracked estimator is pytest-benchmark's ``min``, not its ``mean``: every round performs the same fixed batch of work, so the fastest round is least perturbed by the shared, virtualised runners. ``scripts/normalize_benchmark_json.py`` rewrites ``ops``/``mean`` accordingly, since the action's pytest extractor reads only ``stats.ops``. The trend chart's alert threshold is measured rather than guessed - ``scripts/benchmark_noise.py`` / ``make benchmark-noise`` repeat the measurement on unchanged code - and set to 180%, from the observed 134-158% cross-machine spread plus headroom. At that level it catches only a catastrophic regression, which is all a chart spanning different CPUs can do; the same-runner pull request comparison is the meaningful gate, and alerting stays non-blocking (``fail-on-alert: false``) so ``master`` is never blocked on which machine it drew. ``make benchmarks-ci`` reproduces the CI measurement locally; ``CONTRIBUTING.md`` documents how to read the reports and that local numbers are not comparable to CI numbers
* the tracked set is globally representative rather than worst-case: ``test_timezone_at[random-in_memory]`` (uniformly random query points, containing unique- and ambiguous-shortcut queries in their real ~25.5% ratio) is the headline, with ``unique_shortcut-in_memory`` and ``ambiguous_shortcut-in_memory`` alongside it as diagnostics. An ambiguous lookup costs ~14x a unique one on the tracked configuration and so dominates the headline, which is why the per-class metrics are needed to attribute a unique-path win. ``BATCH_SIZE`` raised from 1,000 to 2,500 for per-round resolution (the lever that bought the tighter threshold), documented against the fixture-size ceilings it must stay under
* guarded the benchmark plumbing against silent drift: benchmark node ids are pinned by ``tests/test_benchmark_names.py`` so a rename fails loudly instead of resetting the trend history; ``tests/test_benchmark_workflows.py`` asserts that the constants duplicated across the two workflows (``REPORT_FILENAME`` and the artifact names) actually agree, where a one-sided edit previously had no failure mode at all and would compare against nothing, and that only the trend chart still goes through ``benchmark-action/github-action-benchmark``, so the cross-machine baseline cannot creep back into the pull request comparison; and ``docs/benchmark_results_*.rst`` carry a "Benchmark Input Provenance" stamp naming the fixture and timezone data versions the timings were measured against, so a report can no longer describe a workload that no longer exists
* ``update_data.sh`` now runs ``make reports`` after regenerating the boundary data, ``DATA_VERSION`` and the benchmark fixtures, so ``docs/benchmark_results_*.rst`` and ``docs/data_report.rst`` no longer go stale relative to the data an update PR ships. A standalone ``make parse``/``make testparse`` still needs a manual ``make reports`` afterwards, since only ``update_data.sh`` guarantees the fixtures are back in sync with ``DATA_VERSION``
* the data pipeline now emits output that is already pre-commit-clean, so regenerating and diffing compares like with like. ``write_json`` sorts keys the way ``pretty-format-json`` does (stringified, since sorting the registry's int keys directly gives numeric rather than lexicographic order) and ``scripts/reporting.py`` no longer emits trailing whitespace on empty table cells or a trailing blank line. Previously every ``make parse``/``make reports`` left ``hole_registry.json`` and ``docs/data_report.rst`` looking modified until the hooks had run, which masked whether a re-parse had actually changed anything
* ``make flatbuf`` no longer overwrites hand-maintained ``__init__.py`` files. ``flatc`` derives its output path from the schema namespace and writes an empty ``__init__.py`` at every level of it, so generating with ``-o .`` wiped the ``__all__`` in ``timezonefinder/__init__.py`` (the whole public API), ``flatbuf/__init__.py`` and ``generated/__init__.py``. The target now generates into a scratch tree and copies back only the three generated packages. It also runs the pre-commit formatters on the result, so ``git diff`` after regenerating shows the codegen change instead of whole-file formatting churn
* mypy now type-checks the whole package except the ``flatc``-generated bindings. ``ignore_errors`` previously also covered ``coord_accessors``, ``polygon_array``, ``np_binary_helpers``, ``utils_numba``, ``utils_clang``, ``_numba_replacements`` and ``flatbuf.io.*``, which silenced *every* type error in roughly 800 lines of hand-written code - a blatantly wrong return type in those modules still reported "Success". They all pass once the exemption is lifted, bar two genuine findings now fixed: the coordinate accessors declared ``__getitem__(idx: int)`` while the lookup path passes numpy integers straight out of the shortcut arrays (widened to ``IntegerLike``; converting instead would have cost a call per candidate polygon), and ``FileCoordAccessor.coord_file`` was annotated ``object``. ``tests/test_mypy_config.py`` keeps the exemption list restricted to generated code, so silencing a module is a reviewed decision rather than a one-line edit
* the hybrid shortcut reader and writer now select their FlatBuffers schema from a single registry (``SHORTCUT_SCHEMAS`` in ``timezonefinder/flatbuf/io/hybrid_shortcuts.py``) instead of dispatching on the zone id width three separate times, each keyed differently: the file name was built from the dtype's ``itemsize``, the writer chose between two 22-line blocks of aliased symbols on ``itemsize``, and the reader recovered the width from a substring of the very file name the first function had produced. The width, the file name, the ``uintN`` marker and the maximum zone id were consequently written down across five places with nothing tying them together, and adding a third width meant finding all of them. One ``ShortcutSchema`` per width now owns all four, derived from its ``zone_id_dtype``; the two generated packages export identical symbols and differ only in ``UniqueZone.zone_id``, so binding their modules replaces the aliasing wholesale. Verified behaviour-preserving down to the bytes: re-writing the shipped 41,162-entry shortcut binary produces a byte-identical file, and ``test_generated_file_name_round_trips`` now covers the writer/reader file name contract, which no test exercised
* removed constructs that provably did nothing, and gave two vacuous tests real assertions. ``AbstractTimezoneFinder.__init__`` read ``zone_ids.npy`` twice, discarding the first array after taking its dtype; four ``__slots__`` entries (``in_memory``, ``_fromfile``, ``_boundaries_file``, ``_holes_file``) were declared but assigned by nothing, which silently re-permitted the very attributes ``__slots__`` is there to forbid - assigning those undocumented names on an instance now raises ``AttributeError``, and ``test_declared_slots_are_assigned`` keeps the list honest; ``MemoryCoordAccessor.cleanup`` guarded a ``.clear()`` behind a ``hasattr`` check that its own preceding ``del`` had already made false, under a comment contradicting the code. On the test side, ``test_runtime_handling_of_single_element_arrays`` asserted against a mock it defined itself, exercising no library code at all, and named a method that no longer exists; it now drives ``timezone_at``, ``unique_timezone_at`` and ``certain_timezone_at`` over a real single-element shortcut cell. ``test_auto_detection_from_filename`` compared two identical payloads to each other, which held for any detection outcome, and is replaced by coverage of the previously untested rejection of a file name carrying no ``uintN`` marker. ``scripts/reporting.py`` derives the hole registry path from ``get_hole_registry_path`` instead of retyping the file name
* added a ``code-quality-pass`` coding-agent skill (``.claude/skills/``, the only part of ``.claude/`` that is tracked) driving one autonomous internal-quality pass: triage, a single-theme refactor that must not change observable behaviour, the full verification gate and a pull request against ``master``. Findings accumulate in ``potential-improvements.md`` at the repository root - committed, so it reaches the next pass through ``master`` - which every pass reads before touching a source file; entries carry a status, so a candidate already shipped or rejected is not raised again and a pass that finds nothing worth changing still leaves its triage behind. The skill deliberately names no files or suspected defects: an example in the instructions would anchor every run on the same handful of findings, which is what the ledger is for
* ``get_corrected_hex_boundaries`` exists once again. The refactor that introduced ``scripts/hex_utils.py`` copied it out of ``scripts/shortcuts.py`` without removing the original, leaving two verbatim copies of the antimeridian and pole clipping rules with nothing keeping them in sync; the ``shortcuts.py`` copy had no callers and is deleted. ``scripts/configs.py`` no longer declares ``MAX_LAT``/``MAX_LNG`` as a second pair of names for ``timezonefinder.configs``'s ``MAX_LAT_VAL``/``MAX_LNG_VAL``. The surviving copy asserted on the latitude span while reporting a "longitude difference", and is now covered by ``tests/hex_utils_test.py`` - it had no direct tests at all, only indirect exercise through ``Hex.from_id`` during data compilation
* added ``potential-improvements.md``, a triaged backlog of internal quality findings (discarded error diagnostics, docstrings describing removed parameters, dead definitions, annotations that contradict their call sites), each with its location, why it matters and what it would cost. Ordered by expected value per line of review; purely a working document, nothing user-facing
* memory is measured by its own harness (``scripts/measure_memory.py``, ``make memory``) rather than by ``pytest-benchmark``, which times code and would have its timings distorted by allocation tracking. Each configuration is measured in a fresh subprocess, since importing NumPy and H3 costs more than any finder and only a delta against a post-import baseline means anything. The harness emits pytest-benchmark-shaped JSON, so ``normalize_benchmark_json``/``benchmark_noise``/``compare_benchmark_runs``/``describe_benchmark_machine`` all work on it unchanged once given a ``--metric`` telling them the units; CI therefore gets the same treatment timings already had - a second trend chart on the existing gh-pages page (``tool: customSmallerIsBetter``, since the pytest extractor tracks iterations per second, where bigger is better) and a same-runner base/head table in the pull request comment. The memory steps run *after* every timing step in the shared job, because reading the boundary data warms the OS file cache that ``benchmarks/test_initialization.py`` disables warmup to avoid. Only the ``tracemalloc`` metrics are charted, at a 110% alert threshold: ``make memory-noise`` measures their spread across five runs at exactly 100.0%, while RSS varied 102-111% because page residency depends on machine-wide pressure. ``tests/test_memory_metric_names.py`` pins the metric names (a rename orphans a chart's history, exactly as for benchmark node ids) and ``tests/test_memory_footprint.py`` fails if a mode's allocation leaves its order of magnitude - the regression that would make ``in_memory=False`` stop being the low-memory option
* ``BenchmarkReporter`` no longer emits a trailing blank line, so ``docs/benchmark_results_*.rst`` are pre-commit-clean as generated. Previously every ``make reports`` left all of them looking modified until ``end-of-file-fixer`` had run, which is the invisible repair the generated-file invariant exists to prevent - the same normalisation ``scripts/reporting.py`` already applied to the data report
