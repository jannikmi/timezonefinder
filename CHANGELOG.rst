...
* Added the exported ``NO_ZONE_ID`` sentinel used by batch ID lookups when ``on_invalid="skip"`` leaves an invalid coordinate in place. ID-taking methods reject the sentinel instead of treating it as a negative index. Solves issue #499.
* Batch lookups accept ``on_invalid="skip"`` to preserve valid results while marking invalid coordinates; the default continues to raise. Non-numeric inputs raise ``TypeError``. Solves issue #499.
...
* ``get_geometry()`` and ``get_polygon()`` decode packed coordinates into returned arrays rather than returning views into stored coordinate buffers.
* Boundary and hole collections use the same packed geometry path in both memory modes. Solves issue #536.
...
* ``is_valid_lat``, ``is_valid_lng``, and ``coord2int`` are ordinary Python functions rather than one-operation Numba dispatches.
* Candidate loops read each polygon's zone ID directly instead of gathering the full candidate list into a temporary NumPy array.
* ``HoleArray`` owns hole lookup, union bounds, and membership checks through ``ids_of`` and ``any_contains``.
* Hole ownership lookup returns an empty range directly rather than using a generator that catches ``KeyError``.
* ``FileCoordAccessor.cleanup()`` releases its own word view before closing the mapping; caller-held views may still defer the close.
* ``PolygonArray`` captures the point-in-polygon kernel alongside the buffers it serves, allowing different acceleration paths to coexist in one process.
* ``certain_timezone_at`` uses the shared shortcut candidate iterator instead of maintaining a second dispatch implementation.
* ``ShortcutIndex`` owns shortcut storage and decoding instead of exposing five arrays and duplicated bit arithmetic to finder methods.
* Shortcut slot addressing uses the contiguous H3 base-cell-and-digit bit field directly.
* Scalar and batch lookup paths share one candidate-polygon loop.
* Batch lookup preparation is memoized by shortcut entry rather than sorting coordinates by cell.
* ``ShortcutIndex.entries_of`` is the batch counterpart of ``entry_of`` and is checked against it across the complete shortcut index.
* Batch answer arrays use the narrowest signed dtype capable of representing every zone plus ``NO_ZONE_ID``; data validation refuses an overflowing zone count.
* Zone-ID-to-name conversion lives in ``ZoneNames`` beside the table it reads; ``timezone_names`` remains a public read-only list view.
* Batch coordinate admission lives beside scalar coordinate validation and preserves their intentional treatment of ``None``.
* Coordinate bounds are declared once in ``timezonefinder.configs`` and reused by validators and diagnostics.
* ``get_last_change_idx`` lives beside the shortcut builder, its only caller, and compiled values are rechecked by data validation.
* Coordinate buffers are written with one NumPy vector copy using a dtype shared by writer and reader; values outside that dtype are rejected.
* The converter creates paired boundary and hole output directories at their shared orchestration point.
* Per-polygon zone IDs use the common vector writer.
* ``ParseAccumulator`` carries converter collections and counters through the parse pipeline instead of ten parallel parameters.
* The converter's zone-ID ordering invariant is enforced once and tested together with dtype, maximum-ID, and position-table rules.
* Converter helpers no longer keep dead iterator, reassignment, or self-return constructs that resembled off-by-one errors.
* Shortcut compilation annotations accept the actual list and array forms passed by production and tests.
* Removed unused JSON and pickle helpers from ``scripts.utils``.
* Removed the unused ``i8`` fallback from ``timezonefinder._numba_replacements``.
* Removed an unused test helper that was retained only for possible future use.
* ``Hex.poly_candidates`` no longer hides an impossible uninitialized cache state behind an empty result.
* Hole polygons remain part of the compiled data because zone-level candidate precedence is cyclic and cannot preserve nested-enclave answers without them. Closes issue #513.
* Data updates compare a frozen sample of answers with the committed baseline.
* Data updates exceeding the answer-change threshold are prepared as draft pull requests for human review rather than discarded.
* Data-update answer comparison runs only for the full-with-oceans dataset whose baseline it describes.
* Boundary and hole payload sizes have separate release gates; polygon counts are recorded without gating renames.
* Reduced-dataset expected names are translated through a vendored, digest-checked upstream merged-zone mapping. Solves issue #334.
* Removed the unused hand-maintained reduced-zone mapping.
* Upstream data downloads are checked against the release asset's published size and SHA-256 before parsing.
* Verified source identity is recorded in ``DATA_SOURCE`` alongside ``DATA_VERSION`` only after successful conversion.
* Upstream release API reads retry transient failures and ignore stale credentials for the public endpoint.
* A branch declaring unpublished data can obtain a compiled wheel from ``compile_data.yml`` instead of committing generated binaries.
* Packaged boundary binaries are git-ignored and obtained from the declared ``timezonefinder-data`` wheel by local and CI bootstrap paths.
* ``DATA_BUILD_RUN`` binds tests, benchmarks, builds, and publication to the same compiled data-wheel artifact.
* The data-wheel handoff accepts an artifact only when its packaged inputs match the checkout; otherwise it rebuilds from a published source.
* Data publication uploads the wheel tested by its update pull request rather than an independent second build.
* ``timezonefinder-data`` supports consecutive PEP 440 ``.postN`` releases for rebuilt output from the same upstream boundary release.
* The data package README records a concise summary for every data release, including 3.2026.3.
* The code-release guard verifies that the built wheel's compatible data requirement is served by PyPI before creating the GitHub Release.
* The PyPI publishing job declares both OIDC and checkout permissions.
* ``timezonefinder`` uses PyPI Trusted Publishing instead of a long-lived API token. Solves issue #525.
* Artifact staging is shared by one composite action, with data wheels excluded from the code publisher's staging directory.
* Release tags reuse the successful ``master`` test matrix instead of running it a second time.
* Tag publication verifies a successful ``build`` run for the exact commit on ``master``.
* Packaging checks validate generated wheel and sdist metadata as well as repository-backed essential files.
* Packaging exclusion patterns must match real checkout paths and remain aligned with ``MANIFEST.in``.
* The test build commands target the interpreter running the test suite.
* Release workflow tests classify every script-side guard by the irreversible action it must precede.
* Pull requests use a template covering the change, motivation, and verification.
* Weekly data updates resolve and download one explicit upstream release instead of racing separate “latest” requests.
* Weekly data updates regenerate, open, merge, and publish through distinct data-release workflows.
* ``update_data.sh`` uses non-interactive dataset flags and writes data-release notes automatically.
* Changelog changes are filed as independently mergeable fragments and assembled during a release.
* The build workflow exposes one stable ``ci-ok`` aggregate gate that fails when any required job is skipped, cancelled, or unsuccessful.
* Release-branch pushes render commit-bound benchmark report artifacts automatically.
* Published benchmark pages are rendered on named CI runners and distinguish full-suite reports from the tracked core subset.
* Benchmark and memory commands assert the acceleration path they intend to measure.
* Benchmark reports are generated from isolated plain-install environments rather than the contributor's optional dependency set.
* CI benchmark comparisons report lookup throughput and memory comments use the median statistic.
* The trend chart uses human-readable labels and timezone lookups per second rather than batch iterations per second.
* CI tracks both batch lookup forms across random, unique-shortcut, and ambiguous-shortcut workloads.
* Pull-request benchmark comments are replaced by posting the new commit-bound result before deleting the superseded comment.
* Pull-request benchmark runs are path-filtered to changes that can affect tracked measurements.
* Added a reusable, order-alternating candidate-comparison harness with unresolved outcomes when its estimators disagree.
* Added per-query latency reporting alongside batch-oriented pytest-benchmark reports.
* Added block-filtered point-in-polygon benchmarks for the kernel path used by real lookups.
* Added a reproducible comparison suite against ``tzfpy`` that records the compared package version.
* Added a memory harness that records allocation and resident-set metrics separately.
* Added a query-stage profiling prototype and recorded its scope and limitations in contributor documentation. Solves issue #497.
* Added a whole-query shortcut-resolution prototype whose correctness gate exercises alternative index resolutions.
* The single-resolution prototype prices the serialized layout actually shipped and includes resolution 5.
* ``scripts/tune_block_size.py`` validates candidate block sizes against rotated rings and the packaged layout.
* Benchmark names and memory metric names are pinned so chart-history joins cannot drift silently.
* Generated benchmark reports open with their measurement configuration and direct readers to methodology details.
* Hand-written documentation links to generated reports rather than copying changing measurements.
* The benchmark methodology documents paired candidate comparison and the asymmetric interpretation of stage shares.
* The data-report generator returns RST and accepts an explicit output path instead of relying on redirected stdout.
* The benchmark-report generator returns RST and accepts an explicit output path instead of relying on redirected stdout.
* Parsing custom output writes its data report beside those binaries; only parsing the packaged directory updates the committed report.
* The data report derives the H3 cell count from H3 rather than a finite table with a misleading fallback.
* Shortcut-index report statistics are calculated by four typed, independently testable helpers.
* The data report labels directly resolved cells as requiring no polygon tests rather than as storing zero polygons.
* Report renderers choose labels and prose through named branches and shared parameter labels.
* Coordinate resolution and ground-distance helpers derive documented precision from the dtype and scale.
* Data-format documentation distinguishes source precision from storage capacity and explains accepted floating-point inputs.
* Data-format documentation records why shortcut cells never make boundary geometry redundant.
* The package comparison states when ``tzfpy`` is the more appropriate choice and links measurements to their source.
* ``timezonefinder.flatbuf.schemas.__all__`` contains only attributes the module defines.
* Ruff enforces ``flake8-bugbear`` and ``RUF013`` consistently in direct and pre-commit runs.
* Warning calls identify their caller through ``stacklevel=2``.
* Ruff's prototype exclusions are declared once.
* Ruff rule ``B023`` is enforced for functions that close over loop variables.
* Ruff rule ``B905`` is enforced, with explicit non-strict ``zip`` calls where variable arity is intentional.
* Mypy covers ``tests/`` and the hook installs the stubs those annotations require.
* Optional-dependency import failures explain the stale environment and recommend ``make install``.
* ``make force_update`` synchronizes all dependency groups.
* ``make outdated`` correctly parses tree-formatted package names.
* Pytest class-scoped fixtures use the classmethod form required by future pytest releases.
* Cleanup exception tests are parametrized over the complete suppressed and warned exception sets.
* Cleanup test doubles fail once so natural finalization cannot leak warnings into a later parametrized case.
* CLI tests select application diagnostics by prefix instead of counting unrelated interpreter stderr lines.
* Shared test helpers derive the repository root from ``scripts.configs`` so wheel installs do not resolve it inside ``site-packages``.
