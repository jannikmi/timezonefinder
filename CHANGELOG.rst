...
* the shortcut index no longer misses a timezone polygon whose boundary crosses an H3 cell without either shape's corners falling inside the other. The index is compiled by testing whether a polygon overlaps a cell, and that test only ever compared vertices - a documented simplification, valid while polygons and cells are of similar size. Where it failed, a cell was recorded as uncovered by a zone that in fact covers it, and every lookup in that cell returned a neighbouring zone with nothing to signal the error. The compiler now also tests whether the two boundaries actually cross. Brute-forcing every polygon for 3,000 points sampled uniformly in latitude and longitude, the packaged index went from 7 such points to 4, and the survivors are all above latitude 88, in the cells spanning a pole, where the new test is deliberately skipped because their stored coordinates are corrected rather than planar - that remainder is tracked in ``potential-improvements.md``. Sampled by area instead, which is what a realistic workload looks like, both before and after return no wrong answers in 3,000 points, so this corrects a narrow class of coordinates rather than a broad one. Compiling the data takes about a second longer
