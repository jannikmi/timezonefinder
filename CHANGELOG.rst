...
* a new prototype, ``prototypes/shortcut_file_format_bench.py``, settles which data structure the H3 shortcut index should use, and whether replacing the packaged binary earns the data format version bump and ordered two-distribution release it would cost. It writes three candidate structures, reads each back, and checks all 41,162 cells resolve identically before timing anything. A flat file storing each distinct entry once wins outright - only 6.9% of entries are distinct, since the ocean zones repeat a single zone id across thousands of cells - taking the shortcut file from ~1,530 KiB to ~497, its load from ~390 ms to ~0.7 ms and its footprint from 4.44 MiB to 0.37, with the lookup unchanged inside a real query. Most of the load win is available without any format change, so the prototype's conclusion is to sequence that cheaper fix first. Its refused-options block records the alternatives that lost and why.
