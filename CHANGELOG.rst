...
* the `package comparison <https://timezonefinder.readthedocs.io/en/latest/alternatives.html>`__ against ``tzfpy`` is now backed by a measurement instead of an impression. Both packages answer the same committed query points in the same process on the same machine, and the result is published as a generated page, `Comparison against tzfpy <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_comparison.html>`__. It confirms the speed row - ``tzfpy`` is faster per lookup, by a small single-digit factor on a representative query mix and by more than an order of magnitude on the points that reach a full point-in-polygon test - and refutes the startup row, which claimed ``tzfpy`` starts instantly. It does not: it imports in about a millisecond and then deserialises its index inside the *first query*, which costs about what this package spends importing NumPy and H3 and reading its own index, so the two reach a first answer in comparable time. Startup is therefore no longer listed as a reason to choose either package. The page also now states what that simplification costs, measured rather than asserted, with a chart on logarithmic axes. ``make tzfpy-agreement`` asks the question at a stated distance from a border and sweeps that distance in 1-and-5 steps from this package's own ~1.1 cm coordinate resolution out to a kilometre, because comparing the two over uniformly drawn coordinates returns no disagreements at all and measures only how much of the world is nowhere near a border. The points it draws sit on the set of positions exactly that far from the nearest border - parallel curves along every boundary edge plus the arcs at every vertex - and each one's distance is verified against the whole dataset rather than assumed from the offset it was given. It refuses to report unless both packages carry the same timezone-boundary-builder release, and separates out the genuinely overlapping zones each package resolves by its own rule, which is what makes the remainder attributable to the geometry. On release 2026c against ``tzfpy`` 1.3.3 the two return a different zone for 47.0% of points 1 cm from the border of a land zone, 33.2% at 1 m, 22.8% at 10 m and 5.7% at 50 m, the knee falling where the ~111 m maximum displacement ``tzfpy``'s maintainer states for it puts it. Just under half is the most reachable, since the points sit on both sides of the border and only one side can be the one the other package moved away from. **The curve does not reach zero**: 0.17% of points 100 m from a border and 0.04% a full kilometre from any border still differ, well past that bound, because a displacement bound says nothing about features a simplification removes or merges - small islands in the Paraná river come back as Paraguay rather than Argentina, and ``Indian/Mahe`` and ``Pacific/Tahiti`` as open ocean. ``tzfpy``'s memory footprint remains unmeasured and the page says so
