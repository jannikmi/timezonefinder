...
* the ``code-quality-pass`` and ``roadmap-pass`` coding-agent skills are one ``improvement-pass`` skill, and ``potential-improvements.md`` is now the single register behind it. The split ran along the wrong seam: it made *the skill* decide whether to consult the maintainer, when what decides that is *the item* - an internal refactor has an obvious correct answer and a batch API's return type does not - so the merged skill asks per item rather than per invocation. One pass now takes exactly one item, the highest-ranked one that is eligible (unclaimed, preconditions met, decisions recorded or obtainable, small enough to review), and ends in one pull request; the former ~400-line diff budget is a ceiling that forces slicing rather than a quota to spend, so an item worth 40 lines ends the pass at 40. The register absorbed the roadmap issue that used to hold the ranking, the sequencing rules and the recorded decisions, which closes it: reasoning kept outside the repository goes stale silently, because nothing references it, no check reads it and a reviewer never sees it in a diff - inside, every change to the ranking arrives as a reviewable diff and the decisions that refuse a re-proposal sit next to the entries they constrain. Issues remain where a single item is worked out. Structural work and internal quality are ranked in one list, since there is only one reviewer's attention to spend on them, and a large item awaiting a decision no longer stalls everything behind it - the pass records the questions and moves down the ranking. ``tests/test_improvement_ledger.py`` asserts that the ranking table and the entries name the same items, an invariant that otherwise breaks in a diff where both halves read as correct on their own
