...
* ``docs/benchmarking_methodology.rst`` gains two sections. One covers comparing two candidate implementations of a single stage, which the existing suites do not do and which three different measurement designs got wrong in a row - each of them flattering the newer candidate, by up to 13 percentage points. The other explains reading a stage's share of a query asymmetrically: it bounds what optimising that stage can win and says nothing about what redesigning it can lose, which for the shortcut lookup is at most ~2.7% of a mixed workload against a measured +29% for one plausible redesign.
