...
* a new prototype, ``prototypes/polygon_block_encoding.py``, settles how the packaged polygon coordinates should be stored, and refuses the delta + varint encoding issue #449 proposed. It measures the thing that proposal would have made worse: per-query latency has a long tail - over on-land points the median lookup is 1.0 microseconds and the 99th percentile 39.7 - and that tail is one ray cast across a very large polygon rather than many candidates, query time correlating 0.92 with vertices tested against 0.76 with the number of candidate polygons. Both encodings #449 reached for decode *sequentially*, so each would have put a second pass over every vertex in front of exactly that test. Storing the vertices in fixed blocks instead, each carrying the range of latitudes its edges span, lets the ray-casting test skip whole blocks: an edge can only change the answer if it spans the query latitude, so the skip is exact rather than approximate, and over the query pairs the committed fixtures produce it removes about 98% of the edge tests. Giving each block its own coordinate origin then compresses the payload to about half of what is shipped today without the sequential decode, since every quantity the test computes is a difference and the origin never has to be added back. The script verifies that a block-encoded test is the same function as the shipped kernel - 410 polygons re-encoded and 1,463 real query pairs replayed, with no disagreement and no round-trip loss - and then prices the skip on a whole query rather than leaving it a work count: with the filter compiled into the point-in-polygon kernel, a paired and order-alternated comparison over 41 rounds is 24% faster on a uniformly random workload and 48% on on-land points, winning every round, with the 99th percentile falling from 37.3 to 8.2 microseconds and the median unchanged. It also sweeps where each ring starts, which a converter is free to change because nothing downstream depends on it: beginning a polygon at its southernmost vertex is slightly worse rather than better, since the last block's wrap-around edge then stretches to that extreme, while choosing the start that minimises the blocks' total latitude coverage scans about 4% fewer edges for a two-second build-time search. Nothing in the package changes
