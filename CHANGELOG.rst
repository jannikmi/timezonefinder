...
* Render CI query comparisons in lookups per second and use the median for memory comments so order-sensitive RSS minima do not masquerade as persistent base changes.
...
* Removed the unused hand-derived ``REDUCED_TIMEZONE_MAPPING`` fragment from the test helpers; a reduced-zone mapping comes from upstream or not at all.
...
* ``timezonefinder-data`` versions may carry a PEP 440 ``.postN`` suffix when converter changes produce revised binaries from the same upstream boundary release. Its PyPI README now records every data release with a concise change summary, including the previously undocumented 3.2026.3 release.
...
* ruff enforces the ``flake8-bugbear`` family and ``RUF013`` beside its default rules, so those findings are caught by ``make hook`` and by CI rather than by ad-hoc ``ruff check --select`` runs. Two ``warnings.warn`` calls gained ``stacklevel=2`` so they point at the caller, and what ``prototypes/`` is exempt from is stated once, in ``[tool.ruff.lint]``, so a bare ``uv run ruff check`` and the pre-commit hook cannot disagree.
* ``certain_timezone_at`` enumerates its candidate polygons through ``_iter_boundaries_in_shortcut`` instead of holding a second copy of the shortcut dispatch that only the test suite exercised.
* the data report's shortcut-index statistics are computed in four named steps rather than one pass. ``scripts/reporting.py``'s ``calculate_shortcut_index_stats`` assembles what ``count_shortcut_entries``, ``shortcut_h3_coverage``, ``shortcut_efficiency_metrics`` and ``shortcut_storage_metrics`` return, so each family's denominators are chosen where they can be read and tested rather than among forty statements of one scope, and each is typed by its own TypedDict that ShortcutIndexStats now inherits. The generated ``docs/data_report.rst`` is unchanged
...
* Track both batch lookup forms across the random, unique-shortcut, and ambiguous-shortcut strata in CI, with the trend threshold calibrated across fresh runners.
...
* **BREAKING:** importing ``timezonefinder`` no longer binds its submodules as package attributes. The public names - the two finder classes, the global functions and ``NO_ZONE_ID`` - are resolved on first access (:pep:`562`), so ``dir(timezonefinder)`` answers the documented surface instead of the twenty-odd modules the eager imports dragged in with it. ``import timezonefinder.utils`` still works; what stops working is reaching a submodule as an attribute of the package without importing it. The bare import no longer loads ``numpy``, ``h3`` or the polygon readers, which takes ``import timezonefinder`` alone from ~16.8 MiB resident to ~3.5 MiB - deferred rather than removed, since a caller that constructs a finder pays it at the first attribute access, and the construction and steady-state figures are unchanged
* **BREAKING:** ``TimezoneFinderL`` no longer accepts ``in_memory``, and neither does the shared base class. It loads no polygon data, so there was no access mode for the argument to select: it was accepted in silence while promising a speedup it could not deliver. ``TimezoneFinderL(in_memory=True)`` now raises ``TypeError``, which is what the command line has always answered for ``--in-memory`` with ``-f 3``/``-f 4``. ``TimezoneFinder(in_memory=...)`` is unchanged
* **BREAKING:** the command line interface requires one of the explicit ``query``, ``rows`` and ``validate-data`` commands. The bare-coordinate form (``timezonefinder LNG LAT``) and the ``--stdin`` flag are removed rather than kept as aliases: use ``timezonefinder query LNG LAT`` and pipe records into ``timezonefinder rows``. Each command owns only the arguments it can use, so argparse rejects a cross-mode option structurally instead of the flat parser accepting every flag and rejecting impossible combinations by hand afterwards
* new batch lookups answer many coordinates in one call, on ``TimezoneFinder``, ``TimezoneFinderL`` and as global functions: ``timezone_ids_at()`` and ``timezone_names_at()``, and the land-restricted ``timezone_ids_at_land()`` and ``timezone_names_at_land()``. Coordinates are passed one array per axis - ``lngs=`` and ``lats=``, any 1-D array-like, with a C-contiguous ``float64`` array used without copying - never as a single ``(N, 2)`` array, whose columns would have to be read positionally and whose swapped pair is still a valid coordinate for most of the populated world. A pandas ``Series`` works and answers come back in input order, so ``df["timezone"] = tf.timezone_names_at(lngs=df["longitude"], lats=df["latitude"])`` is the whole of it. The id form is primary and returns one ``int16`` per coordinate, so a pipeline can join, filter or group on ids and pay for names once, over whatever survives, through the new ``zone_names_from_ids()``. Where the scalar methods answer ``None`` the id array holds the new exported constant ``NO_ZONE_ID`` (``-1``), which the id-taking methods reject, so a sentinel fed back in raises rather than selecting a zone from the end. An out-of-range coordinate - ``NaN`` and infinity included - raises by default as the scalar methods do, and ``on_invalid="skip"`` answers the rest of the batch instead, always one answer per input coordinate and in input order; a value that is not a number at all raises ``TypeError``. Measured over the committed fixtures on the C extension in the default memory-mapped mode, ids come back **1.59x** faster per point than the same lookups one at a time on points whose H3 cell a single zone covers, 1.22x on a uniformly random workload and 1.03x on points that reach the polygon tests, because what a batch amortises is per-call overhead rather than geometry. Points sharing an H3 cell also prepare that cell's candidates once between them - worth a further 3.8% (C extension) or 6.3% (Numba) on points that reach the polygon tests, and real batches cluster: 2,000 points around Basel reach 4 distinct candidate lists. The land form is cheaper per point than the scalar method rather than merely equal to it (~0.94 µs against ~1.49 µs over 2,500 random points), since ocean-ness is a property of a zone id and the whole answer array is masked in one operation. The ceiling is ``h3``, which exposes no vectorised cell lookup. ``certain_timezone_at()`` has no batch form: it tests every candidate polygon rather than stopping at the first match. Solves issue #499
* new ``zoneinfo_at()``, ``utc_offset_at()`` and ``localize()`` on both finder classes and as global functions: the zone as a ``zoneinfo.ZoneInfo``, the UTC offset in force at a given moment, and a naive datetime made aware as local time at the coordinate. They resolve the name through the standard library, which applies the inverted ``Etc/GMT±X`` sign convention correctly. Windows ships no timezone database, so ``pip install tzdata`` is required there first. ``examples/aware_datetime.py`` and ``examples/get_offset.py`` are rewritten around the helpers and no longer need an optional dependency; the first keeps a ``pytz`` section, since with ``pytz`` a naive datetime must go through ``tz.localize()`` while ``replace(tzinfo=...)`` silently attaches a historical offset - a split ``zoneinfo`` does not have. Solves issue #502
* new ``timezonefinder validate-data DIR`` command, which gives users of custom compiled data the same exhaustive cross-file check the converter runs over what it writes and the test suite runs over the packaged directory: file schemas, coordinate offset tables and boundary counts, latitude block indexes and the packed payload frames they describe, hole references and registry ownership, shortcut bounds and stop indexes, and the ``zone_positions`` partition with the polygon-zone grouping it describes. Empty or truncated NumPy files are reported as validation errors rather than tracebacks. Validation stays opt-in and never runs when a finder is constructed or a lookup is answered. See the `usage documentation <https://timezonefinder.readthedocs.io/en/latest/1_usage.html#calling-timezonefinder-from-the-command-line>`__. Solves issue #500
* **the packaged timezone data is half the size, and now matches the boundary its source publishes.** ``coordinates.bin`` goes from 63,423,696 to 31,735,692 bytes and the compiled data directory from ~64.6 MB to ~31.9 MB. Each block of the latitude index is its own coordinate frame - an ``int32`` origin plus a bit width per axis - holding vertices as bit-packed residuals against it instead of fixed 32-bit absolutes; the blocks are the ones the index already partitions a ring into, so nothing new is indexed and no query does more work deciding what to read. Residuals are counted in steps of the grid the boundary data is published on, which is exact and accounts for a further ~6.4 MB. Separately, ``coord2int`` truncated toward zero, which is right for a query and wrong for the source: the vertex timezone-boundary-builder states as ``13.358`` was stored 1.1 cm short of it, on 6.27 % of all coordinates. The converter now rounds onto the source's own grid of six decimal places, so ``get_geometry()`` returns the corrected values - **no vertex is dropped and no ring is redrawn**, the scale factor is unchanged at 10\ :sup:`-7`, and a future upstream release carrying a seventh decimal stops the converter rather than being silently rounded away. **This is a data format change**: it requires ``timezonefinder-data`` 3.x, published before this release, and a hand-compiled ``bin_file_location`` directory has to be regenerated. `The data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ describes the layout, and the published `benchmark <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_timezonefinding.html>`__, `memory <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_memory.html>`__ and `data <https://timezonefinder.readthedocs.io/en/latest/data_report.html>`__ reports are regenerated against it
* **lookups are substantially faster, above all in the tail.** Every boundary polygon ships with a latitude block index: its vertices are split into blocks of 128 and the range of latitudes each block's edges span is stored beside the coordinates. Ray casting flips the inside/outside decision only on an edge spanning the query latitude, so a block whose range excludes it is skipped whole - exactly, not approximately - which leaves about 2% of the edge tests over the committed fixtures. Measured on the C extension in the default memory-mapped mode, paired and order-alternated over 41 rounds: **20% less time per lookup on uniformly random points and 42% on on-land points**, winning every round, with the 99th percentile falling from 35.0 to 11.3 µs on on-land points and the 99.9th from 79.3 to 23.0. The median does not move and points answered from the H3 index alone are unaffected - the index only removes work from queries that read geometry. It costs about half a megabyte of packaged data and the same on the heap in either memory mode, while the mapped mode's *resident* set falls by roughly a third once a workload has run, since a skipped block never faults in the pages behind it. One visible consequence: each stored ring now begins at whichever vertex makes its blocks tightest, so ``get_geometry()`` returns the same closed path starting at a different vertex than the upstream GeoJSON did
* **the two memory modes no longer differ on the geometry path.** A candidate polygon costs 528-746 ns against 2,199-2,516 ns before: the point-in-polygon kernels are handed a whole collection and find a ring by where its blocks start - removing five per-call buffer handles on the C extension - rather than a ring sliced out per candidate, and the accessor resolves where every polygon's coordinates begin once when it is built instead of re-walking the FlatBuffers structure in Python on every lookup. The memory-mapped mode's per-candidate penalty is gone rather than reduced: it now reads the same as ``in_memory=True``, where the mapped fetch used to cost ~950-990 ns against ~210. Measured on the same runner against the merge base, an ambiguous-point batch is 11.4 % faster while the uniformly random and unique-shortcut workloads are unchanged. Construction time, coordinates, results and the public API are untouched. Solves issue #536
* **``in_memory=True`` holds ~32.6 MiB against ~62.7 MiB**, since it keeps the packed payload rather than pre-decoded rings and is the same code path as the default mode with a heap buffer instead of a mapping. The default memory-mapped mode allocates ~2.2 MiB against ~1.6 MiB - the per-block frames held resident - while the pages it faults in fall by about a third, so a constrained container holds less overall. ``get_geometry()`` and ``get_polygon()`` now decode rather than returning a view, which costs ~40 µs plus ~24 ns per vertex and is paid by no lookup
* the H3 spatial index moves from resolution 3 to resolution 4, which takes **about a third less time per lookup** over uniformly random points (the released figure is in the `benchmark report <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_timezonefinding.html>`__, since the latitude block index moves it further). Finer cells mean more of them are covered by a single timezone - 89% against 75% - so a uniformly random workload runs 60% fewer polygon tests; queries already answered by a single table read are unchanged, what changed is how many queries are. The cost is memory: the index holds ~1.0 MiB resident against the ~176 KiB resolution 3 would have needed, which matters most to ``TimezoneFinderL``, whose entire footprint this is, and the packaged binary is 596 KiB against 103 KiB. In the default mapped mode the resident set actually *falls*, from ~32 MiB to ~26 MiB after a workload, because far fewer coordinate pages are faulted in. Resolution 5 was measured and refused: each level costs about twenty times more memory per polygon test removed than the last, and its table alone would be 7.8 MiB. The ``unique_shortcut`` and ``ambiguous_shortcut`` benchmark fixtures are a classification *by* this index, so their figures are not comparable across this change; the random and on-land strata are the same points as before and stay comparable
* constructing a finder is dramatically cheaper and its footprint far smaller, because the H3 shortcut index is stored and read in a new format. A ``TimezoneFinder`` goes from roughly four hundred milliseconds to single-digit milliseconds and a ``TimezoneFinderL`` - whose construction was almost entirely this - to a fraction of one, while a ``TimezoneFinderL`` allocates ~1.0 MiB against ~4.5 MiB. That is paid once per instance, so it matters most to the CLI, to scripts, to serverless cold starts, and to concurrent workloads, which this package asks to build one instance per thread. **Query speed is unchanged**: the format changes how a cell's candidate list is stored, never which candidates come back. Where the old file held forty-one thousand individually decoded FlatBuffers entries, the new one is four plain arrays read as they are - a cell's own bits index a table holding the answer directly, three quarters of cells resolve to a single zone and read nothing else, and the candidate lists that remain are stored once each and shared by the cells that repeat them, with the index of where a candidate loop may stop precomputed at build time. What the format assumes is checked where the data is built and over what is committed, never when a finder is constructed. The internal ``shortcut_mapping`` attribute is gone with the dict it held. Solves issue #477
* the shortcut index no longer misses a timezone polygon that covers part of an H3 cell without a corner of either shape falling inside the other. The index is compiled by testing whether a polygon overlaps a cell, and that test only ever compared corners - a documented simplification, valid while polygons and cells are of similar size, which stops holding as cells shrink and the polygons do not. It was wrong on both sides: on an outer ring, a boundary crossing a cell cleanly was not seen, so the cell was recorded as uncovered by a zone that covers it; on a hole, a cell whose corners all sit inside the hole but which sticks out of it had the polygon taken away again - coordinates near the Lesotho border were answered ``Africa/Maseru``, and open Pacific around an atoll came back as ``Pacific/Tahiti`` instead of its ocean zone. A cell straddling the ±180 degree line was a third case: stored as a planar ring it is not a hexagon at all but a self-intersecting shape spanning most of the globe, so every overlap test was answering about that shape. The compiler now tests whether the two boundaries actually cross, on both sides, and rotates a cell at the antimeridian half a turn about the polar axis before testing it, which moves the coordinate plane's cut to the prime meridian; a polygon the new cut would tear keeps the tests it had, since it cannot reach a cell at the antimeridian anyway. Walking the seven interior points of each of the 288,122 cells, all 2,016,842 are now answered by a polygon that contains them, where 8 were not before, and three cells north of latitude 88.5 gain the ocean zone covering them. Sampled by area, which is what a realistic workload looks like, no wrong answers turn up in 3,000 points either way - so this corrects a narrow class of coordinates rather than a broad one. Compiling the data takes about a second longer
* the point-in-polygon C extension no longer overflows on Windows. The crossing test compares two edge slopes as products that need 63 bits, and held them in ``long`` - 64 bits on Linux and macOS but **32 on Windows**, where the products wrapped and a lookup could be answered by the wrong side of an edge, silently. The affected coordinates are ordinary ones: a few degrees of separation in both axes is already past the 32-bit ceiling once scaled. Both kernels now use ``int64_t``. The Numba and pure-Python paths were never affected, since both were already doing this in 64 bits
* the public methods that take an id - ``zone_id_of``, ``zone_ids_of``, ``zone_name_from_id`` and ``zone_name_from_boundary_id`` - now reject a negative id with ``ValueError`` instead of counting from the end of the dataset. Previously ``zone_name_from_id(-1)`` returned the last zone name, so a caller propagating the conventional ``-1`` "not found" sentinel of an index lookup got a plausible timezone back rather than an exception. The lookup path is unaffected: the ids it produces come from the shortcut index and cannot be negative
* the NumPy arrays a finder retains as loaded dataset state are now read-only. Polygon-to-zone ids, bounding boxes, hole references, shortcut-index columns and mapped coordinate-offset tables are publicly reachable implementation data that every later lookup trusts; assigning into one previously succeeded and silently changed subsequent answers. Fresh arrays returned by batch lookup APIs remain writeable
* ``pip install timezonefinder[numba]`` no longer caps NumPy below 2.4. The extra carried a hand-written ``numpy<2.4`` beside ``numba>=0.60,<1``, matching the bound numba declared at 0.63.0; numba has raised its own ceiling twice since, so the cap had become stricter than the numba it was installed with and held back a NumPy that numba itself supports. numba declares its supported range in its own metadata and the extra exists only to install numba, so the resolver enforces the correct ceiling without help. The same pin is gone from the development dependency group
* the Alpine wheel is now tagged ``musllinux_1_2`` rather than ``musllinux_1_1``. pypa stopped building the ``musllinux_1_1`` image in October 2024, when Alpine 3.12 went end of life, so the pin this repository built against could never be refreshed and took no toolchain or security update after that date. ``musllinux_1_2`` needs musl 1.2, which is Alpine 3.13 and newer, and a pip recent enough to recognise the tag (21.3, October 2021). An installer older than either falls back to the sdist and compiles locally, and ``fallible_build_ext`` means that install still succeeds even when the compile does not - slower, never broken. All three libc images are now pinned to the same dated build
* the `use case documentation <https://timezonefinder.readthedocs.io/en/latest/2_use_cases.html>`__ now warns about the inverted ``Etc/GMT`` sign convention: ``Etc/GMT+5`` is UTC-5 and ``Etc/GMT-5`` is UTC+5, so deriving an offset by parsing the returned zone name produces the wrong sign without failing. Since the packaged data covers the oceans, every coordinate at sea returns one of these zones - the signless ``Etc/GMT`` for the UTC+0 band - which makes this reachable from any lookup rather than a corner case. Call ``utcoffset()`` on an aware datetime instead. The section also points at the standard library's ``zoneinfo`` for new code, explains that Windows users must install ``tzdata``, and drops the dead ``pytz.sourceforge.net`` link. Solves issue #503. Thanks to `weed33834 <https://github.com/weed33834>`__ for the PR #538
* the usage documentation now states the exact-boundary contract of the point-in-polygon lookups: a coordinate precisely on a timezone boundary has no guaranteed side, and ``certain_timezone_at()`` may return ``None`` even with the packaged globe-covering data. The exact South Pole is a reproducible case because polygon vertices meet there; ``timezone_at()`` answers ``Antarctica/McMurdo`` by elimination while ``certain_timezone_at()`` finds no containing polygon. This documents existing behaviour rather than changing border answers or the lookup kernel
* the `package comparison <https://timezonefinder.readthedocs.io/en/latest/alternatives.html>`__ against ``tzfpy`` is now backed by reproducible measurements instead of impressions. It corrects the lookup-speed and startup comparison, measures the effect of ``tzfpy``'s boundary simplification with paired border probes, separates overlap policy from unattributable differences, and commits the measured run so its chart can be regenerated without repeating the external comparison. The page carries the methodology, results, limitations and the remaining unmeasured memory comparison

Internal:

* ``make outdated`` reports the packages that can be upgraded instead of always reporting none. ``scripts/check_upgradeable.sh`` read package names off ``uv tree --outdated`` with a pattern anchored at the start of the line, but ``uv tree`` indents every dependency under its tree drawing, so the filtered list held no third-party package at all and the check answered "all outdated packages are constrained by dependencies" for every tree - while 39 packages were upgradeable. Names are taken from the entries uv marks ``(latest: ...)`` now
* the class-scoped fixture in ``tests/main_test.py`` is declared as a ``classmethod``, the form pytest 9.1 deprecated the instance-method spelling in favour of and removes the alternative to in pytest 10. It already assigned to the class, so no test changes behaviour
* ``is_ocean_timezone`` compares a prefix instead of running a regular expression. ``OCEAN_TIMEZONE_PREFIX`` holds no regex metacharacters and ``re.match`` anchors at the start, so ``str.startswith`` is exactly equivalent at roughly a fifth of the cost. The ~250 ns saved is inside the benchmark suite's noise floor, so this ships as a simplification rather than as a measurable speed-up
* the construction benchmark measures ``TimezoneFinderL`` once rather than twice. Its two cases differed only in an ``in_memory`` argument the class never had anything to apply, so the pair reported one measurement as two. The node id becomes ``test_initialization[TimezoneFinderL]``, which starts a new metric history for it
* a branch whose data is not published yet compiles it in CI instead of committing it. Every data-format change declares a ``timezonefinder-data`` version PyPI has never served, so ``make bootstrap`` has nothing to fetch and only the weekly update job could record the ``DATA_BUILD_RUN`` that CI reads instead. The new ``compile_data.yml`` is that producer: dispatched on a branch, it compiles the timezone-boundary-builder release the branch's own ``DATA_VERSION`` names rather than whichever is latest, verifies the archive against the digest the release API publishes, checks the result answers a lookup, and uploads the wheel every consumer already reads. ``update_data.sh`` grew ``--tag`` to pin the release and ``--binaries-only`` to stop after the converter; its unattended default behaviour is unchanged. The wheel that run builds is the one the pull request's matrix installs and the one the release tag publishes, so a format change reaches PyPI through a single build
* the packaged boundary data is no longer carried by this repository. ``packages/timezonefinder-data/timezonefinder_data/data/`` is git-ignored and ``make bootstrap`` obtains it: it fetches the release ``packages/timezonefinder-data/pyproject.toml`` declares, refuses a download whose SHA-256 disagrees with the digest PyPI publishes, refuses a wheel member that would unpack outside the data directory, and unpacks only the dataset. A checkout that has moved to a commit declaring a different data version re-fetches - the case a "the directory exists" check cannot see, which would otherwise mean testing one release's data against another release's code with nothing to signal it. ``make test``, ``make testint``, ``make testall`` and ``make reports`` depend on that check, and the suite asks it once before collection, so a bare or stale directory is reported as a sentence naming ``make bootstrap`` rather than a ``FileNotFoundError`` three frames down. CI obtains the data the same way with one addition: when ``DATA_BUILD_RUN`` names a workflow run whose wheel carries the declared version, the dataset comes from *that* wheel, since on a format-change pull request the published release is the dataset *before* the one under test and fetching it would validate the wrong bytes and pass. **Nothing about installing or using either distribution changes** - the published wheels are identical. What changes is a clone: the ~62 MB dataset is one command away rather than in the checkout, and a regeneration stops adding that much to the history permanently
* the code-release data-dependency guard accepts all platform wheels for one build, but refuses a staging directory whose matching wheel names are malformed or represent more than one ``timezonefinder`` version. A stale or ambiguous wheel can no longer make the release check validate metadata from a version other than the one about to be published
* the data report generator refuses an incomplete data directory and names the missing hole registry or coordinate file, instead of silently publishing legitimate-looking zero-hole figures
* the point-in-polygon benchmark suite times the block-filtered kernel a lookup actually reaches, alongside the bare one it already tracked. ``docs/benchmark_results_polygon.rst`` had gone on reporting a stage that is linear in polygon size after the latitude block index made the shipped one nearly flat - 641 ns against 1.49 µs across the polygon strata, where the bare kernel spans 1.03 µs to 22.6 µs. Both are tracked under separate names: the bare kernel is still what the pure-Python fallback and the build-time geometry helpers run, and the existing benchmark ids keep their trend history
* per-query lookup latency is measured and published, which the pytest-benchmark suites structurally cannot do: one round there is one pass over a 2,500-point batch, so a distribution is averaged away before the tracked estimator sees it. ``scripts/measure_query_latency.py`` (``make latency``) times each query on its own over the committed fixtures and reports p50/p90/p99/p99.9 per point class; the result is rendered as a section of ``docs/benchmark_results_timezonefinding.rst``, and ``docs/benchmarking_methodology.rst`` says why it is published beside the batch estimators rather than instead of them and why it is deliberately not on the trend chart. ``scripts.render_benchmark_reports`` therefore requires ``--latency-json`` alongside ``--benchmark-json``
* ``scripts/tune_block_size.py`` re-runs the block-size sweep behind ``POLYGON_BLOCK_SIZE`` over whatever data is packaged, reporting edges scanned, block range tests and index size per candidate size, each measured against rings rotated for that size. It reports counts rather than times, which do not depend on the machine or the acceleration backend. 128 stands: 64 ties it on work units at twice the index size, and 32, 256 and 512 all cost more
* dropping the hole polygons is refused for good, and a regression gate now holds the answers it would have moved. Holes look redundant - every one is covered by other zones - but only if the covering zone is *reached* before the surrounding one. ``prototypes/hole_precedence_relation.py`` derives the zone precedence relation that would require, from the geometry alone, and finds it **cyclic**: 216 edges over 218 zones, with seven pairs where each zone must precede the other. Every one is a second-order enclave - Baarle-Hertog and its Dutch counter-enclaves, Nahwa inside Madha inside the UAE, the Hopi reservation inside the Navajo Nation - because precedence is a relation between zones while containment is a relation between rings, and two levels of nesting map opposite ring relations onto one zone pair. No candidate ordering satisfies a cyclic relation, and the size-derived rule proposed for it fails separately on 20 of the 216 edges, systematically on the ocean zones. The new ``tests/test_hole_lookup_regression.py`` walks interior points of every packaged hole and checks each answer against the geometry directly rather than against a committed fixture, so it survives a data update; it fails on 1,404 of 6,048 points against hole-free data. `The data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ carries the conclusion. Closes issue #513
* the latitude block index and the payload offset table are checked where they are produced and over what the repository ships, by ``timezonefinder._data_integrity``. ``validate_block_index`` re-derives every ring's block ranges from the stored coordinates; ``validate_payload_offset_table`` checks the table the lookup path addresses packed payloads through against the FlatBuffers reader, for every ring in both coordinate files. Neither kind of violation announces itself at lookup time - a range one unit too narrow drops a real crossing, and an offset landing one word early still yields plausible packed residuals and therefore plausible wrong coordinates - so both are established exhaustively where the data is produced rather than re-derived in every user's process. The frame and payload validator lives in the same installed module: the converter gives ``validate_block_payload`` the source rings for an exact round trip, while validating an existing compiled directory re-derives every frame from the decoded coordinates
* the pull request benchmark comparison reports a second estimator beside the tracked one. Every row carries its ``median`` change next to the ``min`` change and is marked ``unresolved`` where the two land on different sides of the 110% flag. pytest-benchmark already recorded both and the comparison discarded one, which left the table unable to express "no demonstrable difference" - the shortcut structure comparison reported +0.5% on the ``min`` where a round-level sign count said 26 of 61, and the disagreement was the correct reading. The flag itself is unchanged, so what CI passes or reports is the same. A report recording only one statistic drops the column rather than failing the comparison. ``docs/benchmarking_methodology.rst`` states the rule
* the ``benchmark`` workflow's ``pull_request`` trigger is gated by ``paths``, so a push that cannot move the tracked numbers - docs, changelog, unrelated CI config - no longer re-runs the double checkout, double ``uv sync``, double C-extension build and 2-pass timing sandwich. The listed paths cover the package, the data distribution, the benchmark suite, the measurement and comparison scripts, the build inputs, ``Makefile``, and the workflow file itself
* the ``rstcheck`` pre-commit hook now covers ``docs/``, which it excluded. The exclusion was load-bearing rather than merely conservative: a ``.. note::`` running straight into the bullet list after it sat in ``docs/data_format.rst`` on ``master``, rendering that list outside the surrounding one, and rstcheck reports it the moment anything points it at the file - only nothing did, so a non-failing Sphinx warning was the sole signal. ``sphinx`` is installed into the hook environment so the directives it owns are recognised, which is what forced the exclusion; the three that remain invisible are named in a new ``[tool.rstcheck]`` section in ``pyproject.toml`` - ``autoclass`` and ``autofunction``, and ``include``, which rstcheck resolves against the working directory rather than the including file and abandons the whole file over. Both include targets are linted directly. Note what this does *not* buy: rstcheck never resolves roles, so an unknown ``:doc:`` or ``:ref:`` in ``README.rst`` still passes at every report level
* the data format documentation records **why the H3 index makes no boundary polygon redundant**. Three quarters of cells resolve to a single zone id and a query answered from one reads no geometry, which invites dropping the coordinates of any polygon reachable only that way. No such polygon exists - every boundary polygon is a candidate in at least one ambiguous cell - and the reason is structural rather than a property of this release or resolution, since a polygon's rim cells hold area belonging to another zone at any cell size. The section also covers the weaker form of the idea, which is true and still refused: of the polygons ``timezone_at`` returns by elimination without testing, most are the stored ring of some hole and serve the surrounding polygon's hole test, and the remainder are mainland polygons for which a bounding box averages more than twice the real area
* a data release publishes the wheel its update pull request tested, rather than a second build of it. ``check_data_updates.yml`` builds ``timezonefinder-data`` from the data it just regenerated, attaches it to its run and records that run in a new root-level ``DATA_BUILD_RUN``; ``build.yml`` and ``publish_data.yml`` both obtain the wheel through one composite action, which downloads that artefact and builds only when no recorded run offers one. ``publish_data.yml`` already declared that it does not re-validate the data because the pull request's matrix had checked it, and that only held while the two were looking at one wheel. The recorded artefact is used only when every packaged input it carries is identical to the checkout's, since a change landing on master while an update pull request is open is in the tree the tag names but not in a wheel built before it; anything else falls back to building. Either way the wheel is checked to carry a dataset at all, which is the failure that installs cleanly and fails at the first lookup
* the automated data update verifies the archive it downloads before anything parses it. The weekly job's pull request is merged and its PyPI release tagged without a human reading either, and nothing had looked at the ~55 MB upstream archive - so a truncated transfer, a half-written leftover in ``tmp/`` or an asset replaced upstream reached the converter as ordinary input. ``update_data.sh`` now checks the bytes against the byte size and SHA-256 the GitHub release API publishes, and refuses to parse anything that disagrees or publishes no digest; the existing fallback opens a notification issue. The check runs every run and the archive is unpacked every run, since the download is cached and the unpacked copy is not, so what the converter reads always derives from bytes just verified. The verified digest is recorded in a new root-level ``DATA_SOURCE``, installed alongside ``DATA_VERSION`` once the parse succeeds so the two cannot name different releases - which also catches an asset upstream replaced in place, the one substitution a published digest cannot report. Release API reads retry boundedly after transient failures, and a stale ``GH_TOKEN``/``GITHUB_TOKEN`` falls back to an anonymous read instead of failing the update, since the release API is public
* ``get_last_change_idx`` moves out of the runtime modules to sit beside the shortcut index builder, its only caller. Precomputing the value into the shortcut binary removed the last per-query call to it, but the ``@njit`` function stayed in ``timezonefinder/utils_numba.py``, where numba compiled and bound it at import in every process for something no lookup runs. It is now plain Python next to the builder that writes those values, and ``timezonefinder/_data_integrity.py`` re-checks the committed values against that same implementation
* the finder no longer knows how the shortcut index stores anything. It held the index's five arrays as its own attributes and inlined the H3 bit arithmetic and entry decoding at each of five lookup sites, so the storage format was spread across the lookup layer. ``ShortcutIndex`` now owns that: callers ask what a cell resolves to and, if several zones do, for the candidate polygons and where the scan may stop. Measured paired and order-alternated over 61 rounds, the indirection costs nothing resolvable on any of the four query strata - the original flat shape had been chosen on an assumption that was never measured
* the shortcut lookup's slot arithmetic is one shift and one mask rather than two shifts, two masks, a multiply and an add. H3 stores a cell's base cell immediately above its digits, so the expression the index was addressed by is a single contiguous bit field - an identity over any 64-bit value, not a property of the cells that exist, and pinned by a test asserting it over arbitrary integers as well as real ones. Worth ~5% of a unique-zone query, measured paired and order-alternated over 61 rounds. Accelerating the same arithmetic with numba or the C extension was measured and refused: an empty numba call alone costs about two thirds of the entire stage, so no kernel can pay for the dispatch - recorded in ``contributing/improvements/decisions/query-performance-and-shortcut-index-decisions.md``, since it rules out the whole family rather than one attempt
* the candidate-polygon loop that resolves a point in a cell several zones cover exists once rather than twice, and what a cell costs to *prepare* is separable from working it. ``timezone_at`` prepares one cell and works it; a batch prepares each distinct cell once and works every point that landed in it, memoised by shortcut entry rather than sorted by it, so a batch that shares nothing pays a dict lookup per point instead of an ``argsort``. Preparation is 898 ns against 10,228 ns for resolving a whole ambiguous point on the C extension, which is the ceiling on what sharing it can win. ``ShortcutIndex`` gained ``entries_of``, the batch sibling of ``entry_of``, pinned against it over every cell at the shortcut resolution. ``TimezoneFinderL`` shares more, since its answer is a property of the shortcut entry alone: its whole ambiguous resolution is one gather over the distinct entries, worth ~14% of a batch of such points. The answer dtype is ``int16``, the narrowest that holds a zone id, and ``validate_shortcut_index`` refuses a data directory whose zone count outgrows it, so the width is chosen by fit rather than headroom and cannot truncate silently. The single-point path pays one added method call, on the order of 1% of an ambiguous query, bounded by ``test_batch_and_scalar_agree_over_every_committed_point``
* two responsibilities the batch lookups had accumulated in ``timezonefinder/timezonefinder.py`` now live where they belong. Turning a zone id into a name - the string table, the lazily built gather array, both conversion regimes and the threshold between them - moves to ``timezonefinder/zone_names.py`` as a ``ZoneNames`` object, next to the file it reads; ``timezone_names`` stays a documented public list, now a read-only view onto it. Admitting a batch of coordinates moves to ``timezonefinder/utils.py`` beside the scalar ``validate_coordinates``, so the one place the two arities deliberately differ - a ``None`` coordinate, which ``float()`` refuses and ``numpy`` would turn into ``NaN`` - is visible from both. The lookup module drops to the abstract base and the two finder classes, and the added attribute hop is below what a query can resolve
* the data converter writes each polygon's coordinate block in a single numpy copy instead of one ``PrependInt32`` call per coordinate, taking compilation of the packaged boundaries from ~8.6 s to ~0.03 s for byte-identical output. The coordinate dtype is named once in ``timezonefinder/flatbuf/io/polygons.py`` and used by writer and reader alike, because the builder takes a vector's element width from the array it is given: a polygon that is not already ``int32`` - what ``np.array([[0, 1], [2, 3]])`` produces - would otherwise be stored eight bytes per coordinate and read back as ``int32``. A value too large for the type is refused rather than silently wrapped, which is the range check the per-coordinate calls used to perform
* the data converter creates its ``boundaries/`` and ``holes/`` output directories once, in ``write_binary_files``, rather than in each of the two writers below it. The pair is not independent - the reference vector the numpy writer emits addresses the coordinate file the FlatBuffers writer writes - so each creating the directories read as though either could run on its own. The per-polygon zone ids now go through ``store_per_polygon_vector`` like every other vector, so that file is no longer the only one written without its progress line. The compiled output is byte-identical
* the committed benchmark and memory reports are measured on the plain-install configuration - the C extension without Numba, which is what ``pip install timezonefinder`` gives you and what CI tracks - instead of on whatever the developer's environment happens to hold. ``make install`` syncs every dependency group and ``timezonefinder/utils.py`` binds the backend at import preferring Numba whenever it is importable, so every local measurement silently described the Numba path. It is not only lookup timings that moved: importing Numba costs a few hundred milliseconds, an order of magnitude more than everything the new time-to-first-answer comparison exists to compare. ``make benchmarks`` and ``make memory`` now measure in an isolated environment and assert the acceleration path before recording anything
* a new pytest-benchmark suite, ``benchmarks/test_comparison.py``, measures this package head-to-head against ``tzfpy`` - per-query time over the same four committed fixtures the other suites use, plus time to a first answer in a fresh interpreter, which is the only startup measurement fair to a package that loads its index lazily. Each library is called through its own API inside its own loop, since normalising the two signatures behind one wrapper would cost a Python frame against per-query times of a few hundred nanoseconds. The comparison is deliberately not on the CI trend chart: the number is a ratio against a package that releases outside this repository, so a step would as often mean "they shipped" as "we changed" - the measured version is stamped into the report instead. ``tzfpy`` lives in a new ``compare`` dependency group, and the benchmarks collect without it and skip
* ``docs/benchmarking_methodology.rst`` gains two sections. One covers comparing two candidate implementations of a single stage, which the existing suites do not do and which three different measurement designs got wrong in a row - each flattering the newer candidate, by up to 13 percentage points. The other explains reading a stage's share of a query asymmetrically: it bounds what optimising that stage can win and says nothing about what redesigning it can lose, which for the shortcut lookup is at most a few percent of a mixed workload against a measured +29% for one plausible redesign. The register records the tooling gap the same measurements exposed: the pull request benchmark comparison measures base and head in two sequential runs reduced to a single estimator each, so it cannot resolve the low-single-digit effects that three open performance items will need to demonstrate
* a new prototype, ``prototypes/query_stage_profile.py``, attributes a ``timezone_at`` query to its individual stages - coordinate validation, H3 cell lookup, shortcut resolution, candidate bookkeeping, bounding-box rejection, hole tests and the point-in-polygon kernel - across both acceleration backends and both coordinate access modes, so optimisations that were each justified by an assumption about where query time goes can be argued from a measurement. It takes each figure with three instruments chosen to fail differently - exact line hit counts scaled by a signal sampler's time share, a ladder of prefixes of the real lookup, and ``line_profiler`` for line ordering only - because no single one is unbiased at this scale: enabling ``line_profiler`` deoptimises the whole interpreter and inflates a pure-Python stage ~8x against ~1x for time inside the C extension, skewing exactly the geometry-versus-overhead ratio being measured. Its ``FINDINGS`` block is re-measured against the tree this release ships, since a change to the critical path moves every share quoted from it and a stale share reads exactly like a current one; it now reports all four strata, the uniformly random one being the figure to rank on. Solves issue #497
* ``prototypes/single_resolution_bench.py`` prices each candidate H3 resolution in the layout the package actually ships, checking that pricing against the committed binary before reporting anything, where it previously used a hand-rolled per-entry size model matching no real format. That reopened a settled question: resolution 4 had been refused because the index would exceed 10% of the polygon data, which was a property of the old per-entry file - candidate lists deduplicate, so seven times the cells add a sixth more distinct lists and the file goes to about 0.6 MiB, while removing about 60% of the polygon tests a random workload runs. Resolution 5 is built and benchmarked too, and refused: its gains continue (95.4% of cells unique, another 57% fewer candidates) but the index reaches 4.0 MiB on disk and 7.8 MiB resident, 99.7% of it a table whose size the resolution fixes. The data format reference now says so rather than repeating the stale size argument, and the hierarchical multi-resolution prototype and its plotting helper are removed
* a new prototype, ``prototypes/shortcut_resolution_query_bench.py``, swaps a resolution 4 shortcut index underneath the real ``TimezoneFinder.timezone_at`` to settle whether the finer resolution pays for itself on a whole query, paired and order-alternated as this repository's methodology requires. Its correctness gate is what found the overlap defect fixed above: of 8,000 fixture points the two indices disagreed on one, in the Strait of Malacca, and it also turned up the narrower gap near the poles
* the coordinate resolution this package claims is derived rather than restated. ``timezonefinder.utils.coordinate_resolution()`` returns the representable step of any dtype - the ULP of a float, worst at ±180°, or the fixed-point step of an integer read as a scaled coordinate - and ``degrees_to_metres()`` turns it into ground distance at a given latitude, with ``tests/test_coordinate_precision.py`` holding every documented figure to what they compute. Two claims were wrong and are corrected: ``configs.py`` called the 10\ :sup:`7` scaling "microdegree precision" when one unit is a *tenth* of a microdegree, and ``scripts/file_converter.py`` put the worst case at "1cm" when it is ~1.11 cm. ``docs/data_format.rst`` records what the source data carries, which decides whether the stored precision is a ceiling or padding: timezone-boundary-builder publishes six decimal places, so the seventh carries nothing - measurable rather than assumed, since the last decimal digit of all 15,850,626 packaged values is only ever ``0`` or ``9``. Relaxing it would not shrink the layout or speed up a query, since ``int32`` is forced by the ±180° range rather than by the precision. The page also gains the comparison the input-dtype choice rests on: ``float64`` resolves ~3.5 million times finer than the packaged data and represents the scaled value exactly, while ``float32`` is ~153x coarser at the antimeridian (~1.7 m) and cannot hold 180 × 10\ :sup:`7` exactly at all - so a ``float32`` coordinate array is accepted and upcast, but a point nearer a border than ~1.7 m can already have been rounded onto the wrong side of it
* the benchmark comparison on a pull request is posted as a new comment on every push, with the one it supersedes deleted, instead of being edited in place. A conversation orders comments by creation time and an edit does not move one, so the table stayed where the first push had put it - above every commit and review comment made since, which is exactly where an out-of-date table would sit. Nothing failed when that happened; the numbers just read as stale to everyone reviewing. There is still exactly one table per pull request, and it now names the head commit it measured and links the run that produced it. The new comment is posted before the old one is deleted, since the reverse order leaves the pull request with no comparison at all whenever the post fails, and ``tests/test_benchmark_workflows.py`` pins both that ordering and the absence of the in-place edit
* the artefact-staging block in ``build.yml`` is one composite action (``.github/actions/stage-artifacts``) instead of three near-copies. The copies had drifted - two unzipped ``artifact-*.zip`` and one ``artifact-*``, which silently unzips nothing because ``find -exec`` does not propagate its command's exit status - and the drift sat next to the one difference that is deliberate: the PyPI upload job must not stage ``timezonefinder_data-*`` wheels, since it publishes whatever is in ``dist/`` under the code project's trusted publisher. That exclusion is a named input now. The ``publish-pypi`` job also lost four steps nothing had consumed since the data-dependency check moved into ``release``, and is now download, stage, publish. ``tests/test_release_workflows.py`` asserts that no job stages ``dist/`` inline, that the uploading job excludes the data wheel, and that every job using a local action checks the repository out first
* a release no longer runs the tox test matrix twice. It was the entire critical path of ``build.yml`` - four jobs of six to ten minutes each, against under a minute for every wheel, sdist and end-to-end job together - and ran once for the push to ``master`` and again for the tag naming that same commit. The matrix is skipped on tag refs and the publishing job is gated to tag refs only, so ``master`` tests and the tag releases. That also removes the race the release procedure had to work around, where a push to ``master`` created the GitHub Release and its tag on its own. Skipping is backed by a check rather than an assumption: the release job asks the API whether a successful ``build`` run exists for that exact commit on ``master`` and refuses to publish if none does, since the ancestry check proves only that the commit is on ``master``, not that it was ever green. A tag pushed before ``master``'s run finishes therefore fails the release job, which is re-run once that run passes rather than re-tagged
* ``timezonefinder`` is published to PyPI by Trusted Publishing (OIDC) rather than with a long-lived API token, matching how ``timezonefinder-data`` already publishes. The upload job runs in the ``pypi`` deployment environment and exchanges its OIDC identity for a short-lived, project-scoped token, so no publishing credential exists in repository secrets to leak or rotate. This requires a trusted publisher configured for the ``timezonefinder`` project on PyPI; without it the release upload fails - and since a publisher names the workflow *file*, each of the two workflows records that renaming it breaks publishing. The commented-out Test PyPI step, disabled since the project outgrew the index's size limit, is gone. Solves issue #525
* the packaging guard checks the metadata a build *produces* - ``PKG-INFO`` in the sdists, ``METADATA``/``WHEEL``/``RECORD`` in the wheels - for both distributions. Every case of the existing essential-files check names a path that exists in the checkout, so none of it could reach a file with no counterpart there; missing metadata would have surfaced at upload instead, in a release whose version number cannot be reused
* ``timezonefinder.flatbuf.schemas.__all__`` no longer declares three names the module does not define. It listed the ``.fbs`` schema *files* sitting next to the module rather than submodules of it, so ``from timezonefinder.flatbuf.schemas import *`` raised ``AttributeError``. ``tests/test_documented_contracts.py`` now checks every ``__all__`` in the package against the attributes its module actually has, which nothing did before
* ``tests/`` is type-checked by the mypy pre-commit hook, which had excluded it. Eight real disagreements had accumulated: the base test class in ``tests/main_test.py`` never declared the ``test_instance`` its fixture assigns and its tests read, and two class-level knobs were typed by inference from the base's own value, so two subclasses contradicted a type nobody had written; ``built_files`` in ``tests/test_package_contents.py`` promised a ``list[Path]`` while the attribute behind it was only ever inferred as ``None``; ``tests/locations.py`` annotated a ``dict`` literal as ``set[str, str]``; and the memory fixture let an unmeasured metric through as ``None``, which would have failed as a ``TypeError`` naming neither the configuration nor the metric. The hook's stub list gained ``types-PyYAML``, and the guard keeping ``scripts/`` out of the exclude pattern covers ``tests/`` as well
* the CLI test asserting that a too-wide column number is diagnosed once, not once per row, no longer counts the lines on the subprocess' stderr. stderr is shared with the interpreter, which writes there before the CLI runs any code of its own - a free-threaded CPython 3.14 build reports re-enabling the GIL to import h3 - so the count asserted a property of the interpreter and failed on a tree with nothing wrong with it. The test now selects the lines the CLI itself wrote, by the ``error:``/``warning:`` prefixes it gives them
* the test helpers no longer derive the repository root a second time. ``tests/auxiliaries.py`` computed ``PROJECT_ROOT`` from where the ``timezonefinder`` *package* is installed, which is the checkout only under an editable install - from a wheel it resolves to ``site-packages``, taking ``dist/``, the workflow files and the benchmark fixture paths with it. It now imports the constant from ``scripts.configs``, which anchors on the repository and already owned it
* the memory report's metric explanation is chosen by an ``if``/``else`` rather than by a conditional expression hidden at the foot of a twelve-line string. Only the longer branch interpolates the workload size, so which paragraph a reader is editing decides whether it can name that figure at all - and nothing in the prose said so. The rendered report is unchanged
