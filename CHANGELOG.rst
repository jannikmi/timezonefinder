...
* a new prototype, ``prototypes/shortcut_file_format_bench.py``, prices moving the flat shortcut layout into the packaged binary itself rather than building it at load - a data format change, so it has to earn an ordered two-distribution release. It writes three candidate encodings, reads each back, and checks all 41,162 cells resolve identically before timing. The shortcut load drops from ~399 ms to ~0.03 ms and the packaged file shrinks 2.6-9.2x, but most of the load win is available without any format change, so the prototype's conclusion is to sequence the format question after that cheaper fix. It also settles two design points: dispatch on a zone table derived at load rather than on the stored entry length, and keep H3's internal bit layout out of the file format, where it would outlive the reader that depends on it.
