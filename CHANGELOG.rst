...
* the command line interface has explicit ``query``, ``rows`` and ``validate-data`` commands. Each command owns only the arguments it can use, so argparse rejects cross-mode options structurally instead of a flat parser accepting them and repairing the combinations by hand. The old bare-coordinate and ``--stdin`` forms are removed as part of the major release; use ``timezonefinder query LNG LAT`` and pipe records into ``timezonefinder rows``. ``rows`` appends a ``timezone`` column while building the finder once; coordinate columns come from the header or ``--lng-col``/``--lat-col`` and are never guessed by position, each input row produces one output row, and a rejected row warns and makes the run non-zero without ending the stream. ``validate-data DIR`` gives users of custom compiled data the same exhaustive cross-file check the converter runs over what it writes and the test suite runs over the packaged directory: file schemas, coordinate tables and boundary counts, latitude block indexes, hole references and registry ownership, shortcut bounds and stop indexes, plus the monotonic, terminated ``zone_positions`` partition and the polygon-zone grouping it describes. Empty or truncated NumPy files are reported as clean validation errors rather than tracebacks. The implementation is private but ships in the wheel; validation stays opt-in and never runs when a finder is constructed or a lookup is answered. See the `usage documentation <https://timezonefinder.readthedocs.io/en/latest/1_usage.html#calling-timezonefinder-from-the-command-line>`__. Solves issues #500 and #504. Thanks to `weed33834 <https://github.com/weed33834>`__ for the PR #516
