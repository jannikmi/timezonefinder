...
* a new prototype, ``prototypes/shortcut_file_format_bench.py``, settles which data structure the H3 shortcut index should use, and whether replacing the packaged binary earns the data format version bump and ordered two-distribution release it would cost. It writes three candidate structures, reads each back, and checks all 41,162 cells resolve identically before timing anything. On query speed - the reason the index exists - the winner and the current implementation are near enough indistinguishable, within about two percent across four coordinate strata, so the change is justified on its other axes: **30x less memory, a construction that goes from ~396 ms to ~0.07 ms, and a packaged file a fourteenth of its current size**. The structure is built around the observation that three quarters of cells resolve to a single zone and so need read nothing but one table entry: those answers live in the table itself, leaving offsets only for the distinct candidate lists that really exist, and the index of where the candidate loop may stop is precomputed rather than derived per query. Most of the construction win is available without any format change, so the conclusion is to sequence that cheaper fix first, and the refused-options block records the alternatives that lost and why. The shortcut binary ends up at about a fifth of a percent of the packaged distribution, which is where the prototype stops: further squeezing it would be optimising a rounding error against the 60 MB of polygon coordinates beside it.
