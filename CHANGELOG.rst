...
* the candidate-polygon loop that resolves a point in a cell several zones cover exists once rather than twice: ``timezone_at`` and the new batch path both call it, through ``_zone_id_in_ambiguous_cell`` on each finder class, and it returns a zone id which ``timezone_at`` then names. ``ShortcutIndex`` gained ``entries_of``, the batch sibling of ``entry_of``, so the batch path asks the index what a set of cells resolves to instead of indexing its table itself. The extra call costs **+0.5%** of an ambiguous query on the C-extension backend and +0.8% with numba, measured paired and order-alternated over 244 rounds a side - roughly +0.3% of a realistic mixed workload, and the price of not keeping a second copy of the stop-index and untested-last-zone logic
