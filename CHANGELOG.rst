...
* restored the readability and performance narrative that the pytest-benchmark migration had traded away for statistical rigor: ``docs/benchmark_results_*.rst`` tables now show times in a readable unit (ms/µs/ns picked by magnitude, ``scripts/render_benchmark_reports.py``'s ``format_duration()``) with human-friendly rounding (~3 significant figures via ``_decimals_for_magnitude()`` - ``"414ms"``/``"192x"``/``"3.57M/s"`` rather than ``"414.000ms"``/``"192.30x"``/``"3,573,050/s"``) instead of scientific notation or a fixed decimal count everywhere, plus derived Time/Query and Throughput columns for the timezone-finding and point-in-polygon suites; a new "Performance Summary" section on all three pages states explicit percentage/×-speedup comparisons (in-memory vs file-based, clang vs Python, fastest vs slowest) computed directly from the JSON, calling out a negligible (<2%) difference as "about the same" instead of declaring an arbitrary winner from noise. ``scripts/reporting.py``'s ``print_rst_table()`` now sizes list-table column widths proportional to content (``compute_column_widths()``) instead of splitting evenly, fixing a stale claim in its own docstring and benefiting ``docs/data_report.rst`` too. All underlying pytest-benchmark statistics (Min/Max/StdDev/Rounds) are unchanged - only formatting and the added narrative. The batch size used to derive Time/Query and Throughput is now recorded into the benchmark JSON itself (``benchmarks/conftest.py``'s ``pytest_benchmark_update_machine_info`` hook) and read back by ``render_benchmark_reports.py`` rather than imported from the current checkout's ``benchmarks.conftest.BATCH_SIZE`` - otherwise rendering an older stored JSON after that constant changes would silently derive those columns (and the advertised batch size) from the wrong number
