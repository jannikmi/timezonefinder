...
* the command line interface has explicit ``query``, ``rows`` and ``validate-data`` commands. ``rows`` reads delimited records from standard input and appends a ``timezone`` column, building the finder once; coordinate columns come from the header or ``--lng-col``/``--lat-col`` and are never guessed by position, each input row produces one output row, and a rejected row warns and makes the run non-zero without ending the stream. Bare coordinates and ``--stdin`` remain compatibility aliases for ``query`` and ``rows``. ``validate-data DIR`` gives users of custom compiled data the same exhaustive cross-file check the converter runs over what it writes and the test suite runs over the packaged directory: file schemas, coordinate tables, latitude block indexes, hole references, shortcut bounds and stop indexes, plus the monotonic, terminated ``zone_positions`` partition and the polygon-zone grouping it describes. The implementation is private but ships in the wheel; validation stays opt-in and never runs when a finder is constructed or a lookup is answered. See the `usage documentation <https://timezonefinder.readthedocs.io/en/latest/1_usage.html#calling-timezonefinder-from-the-command-line>`__. Solves issues #500 and #504. Thanks to `weed33834 <https://github.com/weed33834>`__ for the PR #516
