...
* a new prototype, ``prototypes/shortcut_file_format_bench.py``, prices moving the flat shortcut layout into the packaged binary itself rather than building it at load - a data format change, so it has to earn an ordered two-distribution release. It writes five candidate encodings, reads each back, and checks all 41,162 cells resolve identically before timing. The shortcut load drops from ~377 ms to ~0.03 ms and the packaged file shrinks 2.6-10.4x, but most of the load win is available without any format change, so the prototype's conclusion is to sequence the format question after that cheaper fix. Its largest lever is that only 6.9% of shortcut entries are distinct - the ocean zones repeat a single zone id across thousands of cells - so storing each distinct entry once collapses the payload sevenfold at no cost to the common lookup path. It also settles three design points: dispatch on a zone table derived at load rather than on the stored entry length, keep H3's internal bit layout out of the file format where it would outlive the reader that depends on it, and store the cell ids rather than re-enumerating them from H3 at load.
