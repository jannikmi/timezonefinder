...
* the dataset version is now exposed at runtime. ``TimezoneFinder().data_version`` (and ``TimezoneFinderL().data_version``) return the timezone-boundary-builder release the packaged data was built from, read from a ``data_version.txt`` stamp that ``scripts/file_converter.py`` writes into the data directory and that ships in the wheel. ``timezonefinder.__version__`` is now exposed as well, read from the installed distribution metadata. Previously an installed ``timezonefinder`` could state neither - the dataset version lived only in a repo-root file that is not packaged, and the package had no ``__version__`` attribute, so every generated benchmark report carried ``timezonefinder_version: "Unknown"``. Solves issue #498
