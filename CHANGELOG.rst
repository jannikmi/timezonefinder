...
* added continuous benchmarking on CI (solves issue #150), deliberately kept out of the release/publish pipeline in ``build.yml``: ``.github/workflows/benchmark.yml`` runs the tracked core subset on every pull request and every push to ``master``, records the result via ``benchmark-action/github-action-benchmark`` and publishes a trend chart to ``gh-pages``. The pull request job holds no write permissions and no secrets, so branch and fork PRs behave identically; the comparison comment is posted by the separate, privileged ``.github/workflows/benchmark-comment.yml`` via ``workflow_run`` (``pull_request_target`` would grant write permissions in the same context as untrusted PR code). CI tracks the no-numba/clang C extension configuration - what a plain ``pip install timezonefinder`` gives you - and asserts the active acceleration path (``scripts/assert_acceleration_path.py``), since a silent numba install would corrupt the whole trend history
