...
* the `package comparison <https://timezonefinder.readthedocs.io/en/latest/alternatives.html>`__ against ``tzfpy`` is now backed by a measurement instead of an impression. Both packages answer the same committed query points in the same process on the same machine, and the result is published as a generated page, `Comparison against tzfpy <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_comparison.html>`__. It confirms the speed row - ``tzfpy`` is faster per lookup, by a small single-digit factor on a representative query mix and by more than an order of magnitude on the points that reach a full point-in-polygon test - and refutes the startup row, which claimed ``tzfpy`` starts instantly. It does not: it imports in about a millisecond and then deserialises its index inside the *first query*, which costs about what this package spends importing NumPy and H3 and reading its own index, so the two reach a first answer in comparable time. Startup is therefore no longer listed as a reason to choose either. The page also states what the simplification costs, measured rather than asserted, with a chart on logarithmic axes. ``make tzfpy-agreement`` samples the whole offset locus at stated distances from a border, verifies each distance against the whole dataset, requires the same boundary release on both sides, separates genuine overlapping-zone policy, and withholds differences this package cannot attribute. On release 2026c against ``tzfpy`` 1.3.3 the two return a different zone for 47.0% of points 1 cm from the border of a land zone, 33.3% at 1 m, 23.0% at 10 m and 5.7% at 50 m, falling to 0.15% at 100 m and to nothing observed beyond it - the knee landing where the stated ~111 m maximum displacement predicts. Fifteen cases at 1 cm are withheld as exact-grid boundary ambiguity rather than misreported as either package's error; exhaustive scans find no containing packaged polygon and the shortcut candidates are complete. Every attributable disagreement 100 m or more from a border is listed by coordinate, and the whole run is committed as ``docs/tzfpy_agreement.json`` rather than only rendered. ``tzfpy``'s memory footprint remains unmeasured and the page says so
