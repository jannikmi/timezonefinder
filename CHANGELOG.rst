...
* the locked ``cffi`` moves from 2.0.0 to 2.1.1, which is the first release carrying ``abi3t`` - the free-threaded stable ABI. Nothing about the built C extension changes; the bump is what lets the free-threaded packaging arithmetic in issue #364 be settled against a ``cffi`` that supports it
