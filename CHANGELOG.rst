...
* **BREAKING:** importing ``timezonefinder`` no longer binds its submodules as package attributes. The public names - the two finder classes, the global functions and ``NO_ZONE_ID`` - are now resolved on first access (:pep:`562`), so ``dir(timezonefinder)`` answers the documented surface instead of the roughly twenty modules the eager imports happened to drag in with them. ``timezonefinder.utils.validate_coordinates`` was as importable as the documented API while carrying no stability promise. ``import timezonefinder.utils`` still works and still binds the submodule; what stops working is reaching one as an attribute of the package without importing it. The bare import is also cheaper, since it no longer loads ``numpy``, ``h3`` or the polygon readers - ``docs/benchmark_results_memory.rst`` puts what ``import timezonefinder`` alone costs at ~3.5 MiB resident against ~16.8 MiB before. That cost is *deferred* rather than removed: a caller that goes on to construct a finder pays it at the first attribute access, and the construction and steady-state figures on that page are unchanged
* **BREAKING:** ``TimezoneFinderL`` no longer accepts ``in_memory``, and neither does the shared base class. It loads no polygon data at all, so there was no access mode for the argument to select: it was accepted in silence while promising a speedup it could not deliver. ``TimezoneFinderL(in_memory=True)`` now raises ``TypeError``, which is what the command line has always answered for ``--in-memory`` with ``-f 3``/``-f 4``. ``TimezoneFinder(in_memory=...)`` is unchanged
* **BREAKING:** the command line interface requires one of the explicit ``query``, ``rows`` and ``validate-data`` commands. The bare-coordinate form (``timezonefinder LNG LAT``) and the ``--stdin`` flag are removed rather than kept as aliases: use ``timezonefinder query LNG LAT`` and pipe records into ``timezonefinder rows``. Each command owns only the arguments it can use, so argparse rejects a cross-mode option structurally instead of a flat parser accepting every flag and rejecting the impossible combinations by hand afterwards - ``--lng-col`` on a single query, a positional coordinate beside ``--stdin``, and ``--in-memory`` with ``-f 3``/``-f 4`` each needed their own check
* new ``zoneinfo_at()``, ``utc_offset_at()`` and ``localize()`` on both finder classes and as global functions: the zone as a ``zoneinfo.ZoneInfo``, the UTC offset in force at a given moment, and a naive datetime made aware as local time at the coordinate. They resolve the name through the standard library, which applies the **inverted** ``Etc/GMT±X`` sign convention correctly - ``Etc/GMT+5`` is UTC-5, and since the packaged data covers the oceans every coordinate at sea returns one of these zones, so deriving an offset from the returned name produces the wrong sign without failing. Windows ships no timezone database, so ``pip install tzdata`` is required there before any of these resolve a name. ``examples/aware_datetime.py`` and ``examples/get_offset.py`` are rewritten around the helpers rather than hand-rolling the conversion with ``pytz``; the first keeps a ``pytz`` section, since with ``pytz`` a naive datetime must go through ``tz.localize()`` while ``replace(tzinfo=...)`` silently attaches a historical offset - a split ``zoneinfo`` does not have. Neither example needs an optional dependency to run any more. Solves issue #502
* new ``timezonefinder validate-data DIR`` command, which gives users of custom compiled data the same exhaustive cross-file check the converter runs over what it writes and the test suite runs over the packaged directory: file schemas, coordinate offset tables and boundary counts, latitude block indexes and the packed payload frames they describe, hole references and registry ownership, shortcut bounds and stop indexes, plus the monotonic, terminated ``zone_positions`` partition and the polygon-zone grouping it describes. Empty or truncated NumPy files are reported as clean validation errors rather than tracebacks. The implementation is private but ships in the wheel; validation stays opt-in and never runs when a finder is constructed or a lookup is answered. See the `usage documentation <https://timezonefinder.readthedocs.io/en/latest/1_usage.html#calling-timezonefinder-from-the-command-line>`__. Solves issue #500
* ``timezone_at_land()`` now has a batch form: ``timezone_ids_at_land()`` and ``timezone_names_at_land()``, on ``TimezoneFinder``, ``TimezoneFinderL`` and as global functions, with the same one-array-per-axis arguments and the same ``on_invalid`` policies as the existing pair. The ocean check costs nothing per point: ocean-ness is a fixed property of a zone id, so the whole answer array is masked in one operation rather than testing each answer's name - which makes the batch form cheaper per point than the scalar method, not merely equal to it: over 2,500 uniformly random points the batch answers in ~0.94 µs each against ~1.49 µs for ``timezone_at_land()`` in a loop, and the land mask itself is not distinguishable from the plain batch. An ocean match, an uncovered point and a skipped coordinate are all ``NO_ZONE_ID`` in the id array and ``None`` in the name list. ``certain_timezone_at()`` still has no batch form: it tests every candidate polygon rather than stopping at the first match
* **the packaged timezone data is half the size.** ``coordinates.bin`` goes from 63,423,696 to 31,735,692 bytes and the whole compiled data directory from ~64.6 MB to ~31.9 MB, so a ``timezonefinder-data`` download is half what it was. Each block of the latitude index is now its own coordinate frame - an ``int32`` origin plus a bit width per axis - with the vertices held as bit-packed residuals against it instead of as fixed 32-bit absolutes. The blocks are the ones the index already partitions a ring into, so nothing new is indexed and no query does more work deciding what to read; what changed is what a block holds. Residuals are counted in steps of the grid the boundary data is published on rather than of the finer grid a coordinate is stored in, which is exact and accounts for a further ~6.4 MB. `The data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ describes the layout
* **the packaged boundary now matches the boundary its source publishes.** ``coord2int`` truncates toward zero, which is right for a query and wrong for the source: the vertex timezone-boundary-builder states as ``13.358`` was stored as ``133579999``, 1.1 cm short of it, on 6.27 % of all coordinates. The converter rounds onto the source's own grid of six decimal places now, so those coordinates are exact and ``get_geometry()`` returns the corrected values. **No vertex is dropped and no ring is redrawn** - this is not polygon simplification, it is the removal of a conversion error, and the scale factor is unchanged at 10\ :sup:`-7` with a query still placed on that finer grid and compared against the boundary exactly. A future upstream release carrying a seventh decimal would stop the converter rather than be silently rounded away
* **lookups are faster, and the two memory modes no longer differ on the geometry path.** A candidate polygon costs 528-746 ns against 2,199-2,516 ns before, because the point-in-polygon kernels are handed a whole collection and find a ring by where its blocks start - which removes five per-call buffer handles on the C extension - rather than being handed a ring sliced out per candidate. Measured on the same runner against the merge base, an ambiguous-point batch is 11.4 % faster while the uniformly random and unique-shortcut workloads are unchanged. The memory-mapped mode's per-candidate penalty is gone rather than reduced: it now reads the same as ``in_memory=True``, where the mapped fetch used to cost ~950-990 ns against ~210
* **``in_memory=True`` holds ~32.6 MiB against ~62.7 MiB**, since it keeps the packed payload rather than pre-decoded rings and is the same code path as the default mode with a heap buffer instead of a mapping. The default memory-mapped mode allocates ~2.2 MiB against ~1.6 MiB - the per-block frames held resident, which is what this costs - while the pages it faults in fall by about a third, so a constrained container holds less overall. ``get_geometry()`` and ``get_polygon()`` now decode rather than returning a view, which costs ~40 µs plus ~24 ns per vertex and is paid by no lookup
* **this is a data format change.** It requires ``timezonefinder-data`` 3.x, which is published before this release, and a hand-compiled ``bin_file_location`` directory has to be regenerated. The published `benchmark <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_timezonefinding.html>`__ and `memory <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_memory.html>`__ reports are regenerated against it
...
* the `use case documentation <https://timezonefinder.readthedocs.io/en/latest/2_use_cases.html>`__ now warns about the inverted ``Etc/GMT`` sign convention: ``Etc/GMT+5`` is UTC-5 and ``Etc/GMT-5`` is UTC+5, so deriving an offset by parsing the returned zone name produces the wrong sign without failing. Since the packaged data covers the oceans, every coordinate at sea returns one of these zones - the signless ``Etc/GMT`` for the UTC+0 band - which makes this reachable from any lookup rather than a corner case. Call ``utcoffset()`` on an aware datetime instead. The section also points at the standard library's ``zoneinfo`` for new code, explains that Windows users must install ``tzdata`` because the platform provides no system timezone database, and drops the dead ``pytz.sourceforge.net`` link. Solves issue #503. Thanks to `weed33834 <https://github.com/weed33834>`__ for the PR #538
* the default memory-mapped mode (``in_memory=False``) is significantly faster: fetching a candidate polygon's coordinates no longer re-walks the FlatBuffers structure in Python on every lookup. Each coordinate accessor now resolves where every polygon's coordinates begin and how many there are once, when it is built, and afterwards slices them straight out of the buffer. That removes roughly five microseconds per candidate polygon tested, which is around a fifth to a quarter of any query that reaches the point-in-polygon stage - the mapped mode now performs within a few percent of ``in_memory=True`` rather than roughly a third slower, so the low-memory option costs far less speed than it used to. What is cached is two vectors of integers, not the coordinate arrays themselves: a cached array is a zero-copy export of the memory map and would pin it for the lifetime of the finder, in the very mode that exists to avoid holding the data in memory. The table is ~10 KiB for the packaged dataset, and it is built by reading the file rather than the mapping, so a finder that has answered no queries yet has the same resident set it had before - the header words describing the polygons are spread over the whole file, and walking them through the mapping would fault in a page per polygon. Construction time is unchanged. Coordinates, results and the public API are untouched, and queries answered from a single-zone shortcut cell never fetch coordinates and are unaffected. The published `benchmark <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_timezonefinding.html>`__ and `memory <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_memory.html>`__ reports are regenerated against it. Solves issue #536
* the H3 shortcut index is stored and read in a new format, which makes constructing a finder dramatically cheaper and its footprint far smaller. Constructing a ``TimezoneFinder`` goes from roughly four hundred milliseconds to single-digit milliseconds, and a ``TimezoneFinderL`` - whose construction was almost entirely this - to a fraction of one; the index holds a fraction of the memory it used to - a ``TimezoneFinderL`` allocates ~1.0 MiB against ~4.5 MiB - and the packaged binary is well under half its former size. That is paid once per instance, so it matters most to the CLI, to scripts, to serverless cold starts, and to concurrent workloads, which this package asks to build one instance per thread. **Query speed is unchanged**: the index decides which candidate polygons a lookup tests, and the new format changes how a cell's candidate list is stored, never which candidates come back. Where the old file held forty-one thousand individually decoded FlatBuffers entries, the new one is four plain arrays that are read as they are: a cell's own bits index a table holding the answer directly, three quarters of cells resolve to a single zone and read nothing else, and the candidate lists that remain are stored once each and shared by the cells that repeat them. The index of where a candidate loop may stop is precomputed at build time rather than derived per query. Everything the format assumes - that the bit arithmetic still agrees with h3's public accessors, that every column still holds what the dataset puts in it, that the precomputed stop index is right - is checked where the data is built and over what is committed, never when a finder is constructed. **This is a data format change**: it requires ``timezonefinder-data`` 2.x, which is published before this release, and a hand-compiled ``bin_file_location`` directory has to be regenerated. The internal ``shortcut_mapping`` attribute is gone with the dict it held. The published `benchmark <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_initialization.html>`__, `memory <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_memory.html>`__ and `data <https://timezonefinder.readthedocs.io/en/latest/data_report.html>`__ reports are regenerated against it, and `the data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ describes the layout. Solves issue #477

* the shortcut index no longer misses a timezone polygon that covers part of an H3 cell without a corner of either shape falling inside the other. The index is compiled by testing whether a polygon overlaps a cell, and that test only ever compared corners - a documented simplification, valid while polygons and cells are of similar size, which stops holding as the H3 resolution rises and cells shrink while the polygons do not. It was wrong on both sides of a polygon. On the outer ring, a boundary crossing a cell cleanly was not seen, so the cell was recorded as uncovered by a zone that in fact covers it, and every lookup in it returned a neighbouring zone with nothing to signal the error. On a hole - the cut-out an ocean zone has around an island, or one country's enclave inside another - a cell whose corners all sit inside the hole but which sticks out of it had the polygon taken away again, which is the same error reached from the other direction: coordinates in South Africa near the Lesotho border were answered ``Africa/Maseru``, and open Pacific around an atoll came back as ``Pacific/Tahiti`` instead of its ocean zone. The compiler now tests whether the two boundaries actually cross, on both sides. Brute-forcing every polygon for 3,000 points sampled uniformly in latitude and longitude, the outer-ring half took the packaged index from 7 wrong points to 4; the hole half changed 21 cells, 17 of which stopped resolving to a single zone, and of the four million coordinates sampled inside those cells 111 now answer differently - every one of them the answer an exhaustive point-in-polygon test gives. The third way a cell lost its polygon is fixed with them: a cell straddling the +-180 degree line is stored with longitudes that jump from one edge of the coordinate plane to the other, so as a planar ring it is not a hexagon at all but a self-intersecting shape spanning most of the globe - and every overlap test was answering about that shape, which is why the crossing test above was skipped for such cells rather than trusted. The compiler now rotates the cell, the polygon and its holes half a turn about the polar axis before testing them, which moves the coordinate plane's cut to the prime meridian and leaves the cell an ordinary hexagon; a polygon the new cut would tear instead - one sitting on the prime meridian, or wrapped around a pole - keeps the tests it had, since it cannot reach a cell at the antimeridian anyway. Three cells north of latitude 88.5 gain the ocean zone that covers them, and of 600,000 coordinates sampled inside those three, 196,003 now answer differently - every one of them the answer an exhaustive point-in-polygon test gives. Walking the seven interior points of each of the 288,122 cells, all 2,016,842 are now answered by a polygon that contains them, where 8 were not before. Sampled by area instead, which is what a realistic workload looks like, no wrong answers turn up in 3,000 points either way, so this corrects a narrow class of coordinates rather than a broad one. Compiling the data takes about a second longer

* the H3 spatial index moves from resolution 3 to resolution 4, which makes lookups substantially faster: **about a third less time per lookup** over uniformly random points (the released per-lookup figure is in the `benchmark report <https://timezonefinder.readthedocs.io/en/latest/benchmark_results_timezonefinding.html>`__, since the latitude block index below moves it further). Finer cells mean more of them are covered by a single timezone - 89% against 75% - so far fewer queries reach the point-in-polygon stage at all, and a uniformly random workload now runs 60% fewer polygon tests. Queries that were already answered by a single table read are unchanged; what changed is how many queries are. The cost is memory, priced within the new format: the index holds ~1.0 MiB resident against the ~176 KiB resolution 3 would have needed, which matters most to ``TimezoneFinderL``, whose entire footprint this is, and the packaged binary is 596 KiB against 103 KiB - still around one percent of the distribution, and still well below the format it replaces. Construction is 8.3 ms against 8.0 ms. In the default memory-mapped mode the resident set actually *falls*, from ~32 MiB to ~26 MiB after a workload, because far fewer candidate polygons are fetched and so far fewer coordinate pages are faulted in. Resolution 5 was measured too and refused: each level costs about twenty times more memory per polygon test removed than the last, and its table alone would be 7.8 MiB. The ``unique_shortcut`` and ``ambiguous_shortcut`` benchmark fixtures are a classification *by* this index, so they were regenerated and their figures are not comparable across this change - the random and on-land strata are the same points as before and stay comparable.

* the `package comparison <https://timezonefinder.readthedocs.io/en/latest/alternatives.html>`__ against ``tzfpy`` is now backed by reproducible measurements instead of impressions. It corrects the lookup-speed and startup comparison, measures the effect of ``tzfpy``'s boundary simplification with paired border probes, separates overlap policy and unattributable differences, and commits the measured run so its chart can be regenerated without repeating the external comparison. The linked documentation contains the methodology, results, limitations, and remaining unmeasured memory comparison

* new batch lookups ``timezone_ids_at()`` and ``timezone_names_at()`` answer many coordinates in one call, on ``TimezoneFinder``, ``TimezoneFinderL`` and as global functions. Coordinates are passed one array per axis - ``lngs=`` and ``lats=``, any 1-D array-like, with a C-contiguous ``float64`` numpy array used without copying - never as a single ``(N, 2)`` array, whose columns would have to be read positionally and whose swapped pair is still a valid coordinate for most of the populated world. Anything ``numpy.asarray()`` turns into a 1-D numeric array works, a pandas ``Series`` included: answers come back in the input's own order, so ``df["timezone"] = tf.timezone_names_at(lngs=df["longitude"], lats=df["latitude"])`` is the whole of it, and a missing value - which pandas stores as ``NaN`` whether it was written ``None`` or ``pd.NA`` - is an out-of-range coordinate that ``on_invalid`` governs. ``timezone_ids_at()`` is the primary form and returns one ``int16`` timezone id per coordinate, so a caller doing millions of lookups does not pay for millions of string lookups it maps straight back; ``timezone_names_at()`` is the convenience on top of it, and the new ``zone_names_from_ids()`` names a batch of ids at any later point - the batch counterpart of ``zone_name_from_id()`` - so a pipeline can join, filter or group on ids and pay for the names once, over whatever survives. Above ~128 ids that conversion is a numpy gather rather than a Python loop, which is ~3.5x faster per id at a thousand ids and ~5x at ten thousand; below the measured crossover the loop wins and is used instead, and the array it gathers from is built on first use so that constructing a finder allocates nothing for it. Where the scalar methods answer ``None`` - a coordinate no zone covers - the id array holds the new exported constant ``NO_ZONE_ID`` (``-1``), which the id-taking methods now reject, so a sentinel fed back in raises rather than selecting a zone from the end. An out-of-range coordinate, which includes ``NaN`` and infinity, raises by default as the scalar methods do; ``on_invalid="skip"`` answers the rest of the batch instead of discarding the work already done for it, always one answer per input coordinate and in input order. ``on_invalid`` is typed as the two policies it accepts, so a mistyped one is a type error at the call site rather than a runtime ``ValueError``. A coordinate that is not a number at all - a ``None`` in the array, as a nullable column produces - raises ``TypeError`` as the scalar methods do, rather than being read as ``NaN`` and answered under the ``on_invalid`` policy; a ``NaN`` the caller passed deliberately is still an out-of-range coordinate that policy governs. What a batch amortises is the per-call overhead, not the geometry: measured over the committed fixtures on the C-extension backend in the default memory-mapped mode, ids come back **1.59x** faster per point than the same lookups called one at a time on points whose H3 cell a single zone covers, 1.22x on a uniformly random workload and 1.03x on points that reach the point-in-polygon tests. A batch also shares what a single query cannot: points that land in the same H3 cell prepare that cell's candidate polygons once between them rather than once each, which is worth a further **3.8%** (C extension) or **6.3%** (Numba) on points that reach the polygon tests. Real batches cluster - a delivery round, a city, a border region - and 2,000 points scattered around Basel reach just 4 distinct candidate lists between them, against 1,729 preparations before. The ceiling is ``h3``, which exposes no vectorised cell lookup, so N points cost N scalar calls into it whatever the rest of the pipeline does. Only ``timezone_at`` has a batch form; ``timezone_at_land`` and ``certain_timezone_at`` do not. Solves issue #499

* the public methods that take an id - ``zone_id_of``, ``zone_ids_of``, ``zone_name_from_id`` and ``zone_name_from_boundary_id`` - now reject a negative id with ``ValueError`` instead of counting from the end of the dataset. Previously ``zone_name_from_id(-1)`` returned the last zone name, so a caller propagating the conventional ``-1`` "not found" sentinel of an index lookup got a plausible timezone back rather than an exception. The lookup path is unaffected: it reaches the same data through unchecked internal accessors, since the ids it produces come from the shortcut index and cannot be negative.

* the usage documentation now states the exact-boundary contract of the point-in-polygon lookups: a coordinate precisely on a timezone boundary has no guaranteed side, and ``certain_timezone_at()`` may return ``None`` even with the packaged globe-covering data. The exact South Pole is a reproducible case because polygon vertices meet there; ``timezone_at()`` answers ``Antarctica/McMurdo`` by elimination while ``certain_timezone_at()`` finds no containing polygon. This documents the existing behaviour rather than changing border answers or the lookup kernel.

* the point-in-polygon C extension no longer overflows on Windows. The crossing test compares two edge slopes as products that need 63 bits, and it held them in ``long`` - which is 64 bits on Linux and macOS but **32 on Windows**, where the products therefore wrapped and a lookup could be answered by the wrong side of an edge, silently. The affected coordinates are ordinary ones: a few degrees of separation in both axes is already past the 32-bit ceiling once scaled. Both kernels now use ``int64_t``, which is the width the arithmetic requires on every platform. Builds without the C extension - the Numba and pure-Python paths - were never affected, since Python and Numba were already doing this in 64 bits.
* timezone lookups are substantially faster, above all in the tail. Every boundary polygon now ships with a **latitude block index**: its vertices are split into blocks of 128 and the range of latitudes each block's edges span is stored beside the coordinates. Ray casting flips the inside/outside decision only on an edge that spans the query latitude, so a block whose range excludes it cannot change the answer and is skipped whole - exactly, not approximately. Over the query points the committed fixtures produce that leaves about 2% of the edge tests. Measured on the C extension in the default memory-mapped mode, paired and order-alternated over 41 rounds: **20% less time per lookup on uniformly random points and 42% on on-land points**, winning every round, with the 99th percentile falling from 35.0 to 11.3 microseconds on on-land points and the 99.9th from 79.3 to 23.0. The median does not move and points answered from the H3 index alone are unaffected - the index only ever removes work from the queries that read geometry. It costs about half a megabyte of the packaged data, against 63 MB of coordinates, and the same on the heap in either memory mode - but the default mapped mode's *resident* set falls by roughly a third once a workload has run, because a lookup that skips a block never faults in the coordinate pages behind it.

  Two consequences worth knowing. The polygon binary layout moves to version 2, so this needs the ``timezonefinder-data`` 2.x release the pending data-format bump already required - the format generation itself is unchanged. And each stored ring now begins at whichever vertex makes its blocks tightest, which is free to choose because nothing depends on it: ``get_geometry()`` returns the same closed path starting at a different vertex than the upstream GeoJSON did.

* ``pip install timezonefinder[numba]`` no longer caps NumPy below 2.4. The extra carried a hand-written ``numpy<2.4`` beside ``numba>=0.60,<1``, matching the bound numba declared at 0.63.0; numba has raised its own ceiling twice since, so the cap had become stricter than the numba it was installed with and held back a NumPy that numba itself supports. numba declares its supported NumPy range in its own metadata and the extra exists only to install numba, so the resolver enforces the correct ceiling without help. The same pin is gone from the development dependency group

Internal:

* ``make outdated`` reports the packages that can be upgraded, instead of always reporting none. ``scripts/check_upgradeable.sh`` read package names off ``uv tree --outdated`` with a pattern anchored at the start of the line, but ``uv tree`` indents every dependency under its tree drawing and leaves column 0 to the workspace roots and uv's own status banner - so the list it filtered held no third-party package at all and the check answered "all outdated packages are constrained by dependencies" for every tree, whatever was actually stale. It reported that while 39 packages were upgradeable. Names are taken from the entries uv marks ``(latest: ...)`` now
* the class-scoped fixture in ``tests/main_test.py`` is declared as a ``classmethod``, which is the form pytest 9.1 deprecated the instance-method spelling in favour of and removes the alternative to in pytest 10. It already assigned to the class rather than the instance, so no test changes behaviour
* ``is_ocean_timezone`` compares a prefix instead of running a regular expression. ``timezone_at_land()`` calls it once per answer, and ``OCEAN_TIMEZONE_PREFIX`` holds no regex metacharacters while ``re.match`` anchors at the start, so ``str.startswith`` is exactly equivalent at roughly a fifth of the cost. The saving is ~250 ns on a check that is ~6 % of a mixed ``timezone_at_land`` workload - inside the benchmark suite's noise floor, so this ships as a simplification rather than as a measurable speed-up
* the construction benchmark measures ``TimezoneFinderL`` once rather than twice. Its two cases differed only in an ``in_memory`` argument the class never had anything to apply, so the pair reported one measurement as two - the same reasoning the memory harness already applied to its own configurations. The node id becomes ``test_initialization[TimezoneFinderL]``, which starts a new metric history for it
* a branch whose data is not published yet compiles it in CI instead of committing it. Every data-format change declares a ``timezonefinder-data`` version PyPI has never served, so there is nothing for ``make bootstrap`` to fetch and - until now - nothing but the weekly update job could record the ``DATA_BUILD_RUN`` that CI reads instead. The new ``compile_data.yml`` is that producer: dispatched on a branch, it compiles the timezone-boundary-builder release the branch's own ``DATA_VERSION`` names rather than whichever is latest, verifies the archive against the digest the release API publishes, checks the result answers a lookup, and uploads the wheel every consumer already reads. ``update_data.sh`` grew the two flags it needs - ``--tag`` to pin the release and ``--binaries-only`` to stop after the converter, leaving the branch's own stamps, fixtures and report pages alone - and its default behaviour, which the weekly pipeline runs unattended, is unchanged. The wheel that run builds is the one the pull request's matrix installs and the one the release tag publishes, so a format change reaches PyPI through a single build
* the packaged boundary data is no longer carried by this repository. ``packages/timezonefinder-data/timezonefinder_data/data/`` is git-ignored, and ``make bootstrap`` obtains it instead: it fetches the ``timezonefinder-data`` release ``packages/timezonefinder-data/pyproject.toml`` declares, refuses a download whose SHA-256 disagrees with the digest PyPI publishes for it, refuses a wheel member whose name would unpack outside the data directory, and unpacks only the dataset. A second run does nothing, and a checkout that has moved to a commit declaring a different data version re-fetches - which is the case a "the directory exists" check cannot see, and would otherwise mean testing one release's data against another release's code with nothing to signal it. ``make test``, ``make testint``, ``make testall`` and ``make reports`` depend on that check, and the test suite asks it once before collection, so a bare or stale data directory is reported as a sentence naming ``make bootstrap`` rather than as a ``FileNotFoundError`` on a ``.npy`` file three frames down. CI obtains the data the same way, with one addition it cannot do without: when the ``DATA_BUILD_RUN`` stamp names a workflow run whose data wheel carries the declared version, the dataset is taken from *that* wheel. A data update bumps the version to one PyPI will not serve until the release tag publishes, so on that pull request the published release is the dataset *before* the one under test - fetching it would validate the wrong bytes and pass. ``update_data.sh`` restates what the directory holds after it bumps the data version, so the dataset it just regenerated is not read as stale by the very targets that verify it, and the update job now stages the paths it names rather than everything it finds. **Nothing about installing or using either distribution changes** - the published wheels are identical and a ``pip install`` never saw this repository's layout. What changes is a clone: the ~62 MB dataset is one command away rather than in the checkout, and a regeneration stops adding that much to the history permanently
...
* the point-in-polygon benchmark suite times the block-filtered kernel a lookup actually reaches, alongside the bare one it already tracked. Only the bare kernel was measured before, so ``docs/benchmark_results_polygon.rst`` went on reporting a stage that is linear in polygon size after the latitude block index had made the shipped one nearly flat - 641 ns against 1.49 µs from the smallest polygon stratum to the largest, where the bare kernel spans 1.03 µs to 22.6 µs. Both forms are kept and tracked under separate names: the bare kernel is still what the pure-Python fallback and the build-time geometry helpers run, and the existing benchmark ids keep their trend history
* per-query lookup latency is measured and published, which the pytest-benchmark suites structurally cannot do: one round there is one pass over a 2,500-point batch, so a distribution is averaged away before the tracked estimator sees it. ``scripts/measure_query_latency.py`` (``make latency``) times each query on its own over the committed fixtures and reports p50/p90/p99/p99.9 per point class; the result is rendered as a section of ``docs/benchmark_results_timezonefinding.rst``, and ``docs/benchmarking_methodology.rst`` says why it is published beside the batch estimators rather than instead of them and why it is deliberately not on the trend chart. ``scripts.render_benchmark_reports`` therefore requires ``--latency-json`` alongside ``--benchmark-json``, since that section is part of a page it rewrites wholesale
...
* dropping the hole polygons is refused for good, and a regression gate now holds the answers it would have moved. Holes were expected to be redundant - every one of them is covered by other zones, so the surrounding zone's cut-out looks like information the lookup already has - and that is only true if the covering zone is *reached* before the surrounding one. ``prototypes/hole_precedence_relation.py`` derives the zone precedence relation that would require, from the geometry alone and without consulting the H3 shortcut index, and finds it **cyclic**: 216 edges over 218 zones, containing seven pairs where each zone must precede the other. Every one is a second-order enclave - Baarle-Hertog and its Dutch counter-enclaves, Nahwa inside Madha inside the UAE, the Hopi reservation inside the Navajo Nation - because precedence is a relation between zones while containment is a relation between rings, and two levels of nesting map opposite ring relations onto one zone pair. No candidate ordering satisfies a cyclic relation, so there is nothing to enforce and nothing to wait for; the size-derived rule proposed for it fails separately on 20 of the 216 edges, systematically on the ocean zones, which are huge by area and cheap by vertex count. The new ``tests/test_hole_lookup_regression.py`` walks interior points of every packaged hole and checks each answer against the geometry directly rather than against a committed fixture, so it survives a data update; it fails on 1,404 of 6,048 points against hole-free data. ``prototypes/hole_removal_impact.py`` was rebuilding a variant data directory around a symlinked latitude block index and could no longer run at all; it now writes the index with the rings it describes. `The data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ carries the conclusion. Closes issue #513
* the latitude block index is checked where it is produced and over what the repository ships, by ``timezonefinder._data_integrity.validate_block_index``, which re-derives every ring's block ranges from the stored coordinates. Nothing about a wrong range announces itself at lookup time - the kernels skip on it, so a range one unit too narrow drops a real crossing and answers one point differently - and re-deriving it in every user's process is exactly what this repository's integrity checks exist not to do
* the pull request benchmark comparison now reports a second estimator beside the tracked one. Every row of the table carries its ``median`` change next to the ``min`` change, and is marked ``unresolved`` where the two land on different sides of the 110% flag. pytest-benchmark already recorded both and the comparison discarded one, which left the table unable to express the answer "no demonstrable difference" - the shortcut structure comparison it was measured against reported +0.5% on the ``min`` where a round-level sign count said 26 of 61, and the disagreement was the correct reading. The flag itself is unchanged and still derived from the tracked estimator, so what CI passes or reports is the same; ``--fail-on-regression`` remains unused. A report that records only one statistic - an older one, or a metric such as the memory measurement's - drops the column rather than failing the comparison. ``docs/benchmarking_methodology.rst`` states the rule

* the ``rstcheck`` pre-commit hook now covers ``docs/``, which it excluded. The exclusion was silently load-bearing rather than merely conservative: a ``.. note::`` running straight into the bullet list after it sat in ``docs/data_format.rst`` on ``master``, rendering that list outside the surrounding one, and rstcheck reports it the moment anything points it at the file - only nothing did, so the Sphinx build's warning was the sole signal and a warning that does not fail a build is one nobody is obliged to read. ``sphinx`` is installed into the hook environment so the directives it owns (``toctree``, ``automodule``) are recognised instead of reading as unknown, which is what forced the exclusion; the three that remain invisible are named in a new ``[tool.rstcheck]`` section in ``pyproject.toml`` - ``autoclass`` and ``autofunction``, since autodoc is not part of that shim, and ``include``, which rstcheck resolves against the working directory rather than the including file and abandons the whole file over, silently checking nothing in the three pages that use it. Both include targets are linted directly, so skipping the directive covers the same content. Putting the settings in ``pyproject.toml`` rather than in hook arguments means a bare ``rstcheck`` run agrees with the hook. Note what this does *not* buy: rstcheck never resolves roles, so an unknown ``:doc:`` or ``:ref:`` in ``README.rst`` passes at every report level and the standalone-RST requirement for the PyPI long description is still enforced by nothing but rendering the file

* the data format documentation records **why the H3 index makes no boundary polygon redundant**, under a new *Why the index makes no polygon redundant* section next to the hole sections it parallels. Three quarters of the cells resolve to a single zone id and a query answered from one reads no geometry, which invites dropping the coordinates of any polygon reachable only that way and degrading ``get_geometry`` to a bounding box for it. No such polygon exists - every boundary polygon is a candidate in at least one ambiguous cell - and the reason is structural rather than a property of the release or the resolution, since a polygon's rim cells hold area belonging to another zone at any cell size. The section also covers the weaker form of the idea, which is true and still refused: of the polygons ``timezone_at`` returns by elimination without ever testing, most are the stored ring of some hole and serve the surrounding polygon's hole test, the remainder are mainland polygons for which a bounding box averages more than twice the real area, and qualifying at all depends on how the index happened to sort its candidates. In the same page, a ``.. note::`` under *Other Files* is now separated from the bullet list that follows it, which it ran straight into - the list rendered outside the surrounding one and the Sphinx build warned about it. Documentation only; nothing in the package changes

* a data release publishes the wheel its update pull request tested, rather than a second build of it. ``check_data_updates.yml`` builds ``timezonefinder-data`` from the data it has just regenerated, attaches it to its run and records that run in a new root-level ``DATA_BUILD_RUN``; ``build.yml`` and ``publish_data.yml`` both obtain the wheel through one composite action, which downloads that artefact and builds only when no recorded run offers one - a checkout regenerated outside the pipeline, which is what ``update_data.sh`` clearing the stamp makes recognisable. ``publish_data.yml`` already declared that it does not re-validate the data because the pull request's matrix had checked it, and that only ever held while the two were looking at one wheel. The recorded artefact is used only when every packaged input it carries - the dataset, the module, the licence - is identical to the checkout's, since a change landing on master while an update pull request is open is in the tree the tag names but not in a wheel built before it; anything else falls back to building. Either way the wheel is then checked to carry a dataset at all, which is the failure that installs cleanly and fails at the first lookup

* the ``benchmark`` workflow's ``pull_request`` trigger is now gated by ``paths``, so a push that cannot move the tracked numbers - docs, changelog, CI config unrelated to this workflow - no longer re-runs the double checkout, double ``uv sync``, double C-extension build and 2-pass timing sandwich. The listed paths cover the package, the data distribution, the benchmark suite, the measurement/comparison scripts the job shells out to, the build inputs (``setup.py``, ``pyproject.toml``, ``uv.lock``), ``Makefile``, and the workflow file itself

* the finder no longer knows how the shortcut index stores anything. It held the index's five arrays as its own attributes and inlined the H3 bit arithmetic and entry decoding at each of the five lookup sites, so the storage format was spread across the lookup layer and could not change without touching it. ``ShortcutIndex`` now owns that: callers ask what a cell resolves to and, if several zones do, for the candidate polygons and where the scan may stop. The encoding stops at that boundary. Measured paired and order-alternated over 61 rounds, the indirection costs nothing resolvable on any of the four query strata - the original flat shape had been chosen on an assumption that was never measured

* the automated data update verifies the archive it downloads before anything parses it. The weekly job's pull request is merged and its PyPI release tagged without a human reading either, and nothing had ever looked at the ~55 MB upstream archive - so a truncated transfer, a half-written leftover in ``tmp/`` or an asset replaced upstream reached the converter as ordinary input. ``update_data.sh`` now checks the bytes against the byte size and SHA-256 the GitHub release API publishes for that asset, which is an independent statement of what they should be, and refuses to parse anything that disagrees or that publishes no digest to check against; the existing fallback opens a notification issue for the maintainer. The check runs on every run rather than only on the one that downloaded the archive, and the archive is unpacked every run too: the download is cached between runs, the unpacked copy is not, so what the converter reads always derives from bytes that were just verified rather than from a file left in ``tmp/``. The verified digest is recorded in a new root-level ``DATA_SOURCE`` file, installed alongside ``DATA_VERSION`` once the parse has succeeded so that the two stamps cannot come to name different releases, so an update pull request states which upstream bytes produced its binaries and a later run over an already-recorded tag catches an asset that upstream replaced in place - the one substitution the published digest cannot report, since it moves with the asset. Release API reads retry boundedly after transient server or transport failures. A GitHub token is used only while it is being accepted: the release API is public, so a stale ``GH_TOKEN``/``GITHUB_TOKEN`` left in a shell falls back to an anonymous read instead of failing the update. Resolving the release tag moved into the same module, replacing a ``grep``/``cut`` over the API's JSON

* ``get_last_change_idx`` moves out of the runtime modules to sit beside the shortcut index builder, its only caller. Precomputing the value into the shortcut binary removed the last per-query call to it, but the ``@njit`` function stayed in ``timezonefinder/utils_numba.py``, where numba compiles and binds it at import in every process for something no lookup runs. It is now plain Python next to the builder that writes those values, and ``timezonefinder/_data_integrity.py`` re-checks the committed values against that same implementation, so the two still cannot drift

* a new pytest-benchmark suite, ``benchmarks/test_comparison.py``, measures this package head-to-head against ``tzfpy`` - per-query time over the same four committed point fixtures the other suites use, plus time to a first answer in a fresh interpreter, which is the only startup measurement that is fair to a package that loads its index lazily. Each library is called through its own API inside its own loop, since normalising the two call signatures behind one wrapper would cost a Python frame against per-query times of a few hundred nanoseconds. The comparison is deliberately not on the CI trend chart: the number is a ratio against a package that releases outside this repository, so a step in that chart would as often mean "they shipped" as "we changed" - the measured version is stamped into the report instead. ``tzfpy`` lives in a new ``compare`` dependency group, and the benchmarks collect without it and skip, so the pinned node id set stays independent of which optional packages are installed

* the committed benchmark and memory reports are now measured on the plain-install configuration - the C extension without Numba, which is what ``pip install timezonefinder`` gives you and what CI tracks - instead of on whatever the developer's environment happens to hold. ``make install`` syncs every dependency group, and ``timezonefinder/utils.py`` binds the point-in-polygon backend at import time preferring Numba whenever it is importable, so every local measurement silently described the Numba path. It is not only the lookup timings that moved: importing Numba costs a few hundred milliseconds, an order of magnitude more than everything the new time-to-first-answer comparison exists to compare. ``make benchmarks`` and ``make memory`` now measure in an isolated environment and assert the acceleration path before recording anything, and all five report pages are regenerated from one run of it

* the shortcut lookup's slot arithmetic is one shift and one mask rather than two shifts, two masks, a multiply and an add. H3 stores a cell's base cell immediately above its digits, so the expression the index was addressed by is a single contiguous bit field - an identity over any 64-bit value, not a property of the cells that exist, and pinned by a test that asserts it over arbitrary integers as well as real ones. Worth ~5% of a unique-zone query, measured paired and order-alternated over 61 rounds. Accelerating the same arithmetic with numba or the C extension was measured and refused: an empty numba call alone costs about two thirds of the entire stage, so no kernel can pay for the dispatch - recorded in ``contributing/improvements/decisions/query-performance-and-shortcut-index-decisions.md``, since it rules out the whole family rather than one attempt

* a new prototype, ``prototypes/shortcut_resolution_query_bench.py``, swaps a resolution 4 shortcut index underneath the real ``TimezoneFinder.timezone_at`` to settle whether the finer resolution pays for itself on a whole query, paired and order-alternated as this repository's methodology requires. Its correctness gate is what found the overlap defect fixed above: of 8,000 fixture points the two indices disagreed on one, in the Strait of Malacca, and resolution 4 was the wrong one there because the shortcut compiler decided polygon/cell overlap by vertex inclusion alone. It also turned up a second, narrower gap near the poles, where cells above latitude 88 omitted the polygon covering them - fixed above, and invisible to a realistic workload either way: sampled by area rather than uniformly in latitude, 3,000 points produce no wrong answer at either resolution. Nothing in the package changes

* ``prototypes/single_resolution_bench.py`` now prices each candidate H3 resolution in the layout the package actually ships, checking that pricing against the committed binary before reporting anything, where it previously used a hand-rolled per-entry size model matching no real format. That reopens a settled question: resolution 4 was refused because the index would exceed 10% of the polygon data, which was a property of the old per-entry file - candidate lists deduplicate, so seven times the cells add a sixth more distinct lists and the file goes to about 0.6 MiB rather than past 10%, while removing about 60% of the point-in-polygon tests a random workload runs. The cost moved to a sevenfold resident table. Resolution 5 is now built and benchmarked too, and refused: its gains continue (95.4% of cells unique, another 57% fewer candidates) but the index reaches 4.0 MiB on disk and 7.8 MiB resident, 99.7% of it a table whose size the resolution fixes - each level costs about twenty times more memory per candidate removed than the last. `The data format reference <https://timezonefinder.readthedocs.io/en/latest/data_format.html>`__ now says so rather than repeating the stale size argument. The hierarchical multi-resolution prototype and its plotting helper are removed - the index format has moved out from under them - and the conclusion they reached is recorded on that page instead

* the data converter writes each polygon's coordinate block in a single numpy copy instead of one ``PrependInt32`` call per coordinate, which takes compiling the packaged boundaries from ~8.6 s to ~0.03 s for byte-identical output - the 15.85 million coordinates were being handed to the FlatBuffers builder one Python call at a time. The coordinate dtype is now named once in ``timezonefinder/flatbuf/io/polygons.py`` and used by the writer and the reader alike, because the builder takes a vector's element width from the array it is given: a polygon that is not already ``int32`` - which is what ``np.array([[0, 1], [2, 3]])`` produces - would otherwise be stored eight bytes per coordinate and read back as ``int32``, and a value too large for the type is now refused rather than silently wrapped, which is the range check the per-coordinate calls used to perform

* the CLI test asserting that a column number wider than the input is diagnosed once, not once per row, no longer does so by counting the lines on the subprocess' stderr. stderr is shared with the interpreter, which writes there before the CLI runs any code of its own - a free-threaded CPython 3.14 build reports re-enabling the GIL to import h3 - so the count asserted a property of the interpreter and failed on a tree with nothing wrong with it. The test now selects the lines the CLI itself wrote, by the ``error:``/``warning:`` prefixes it gives them, and pins those to the single expected error, which is what the case means and what a per-row diagnostic would break

* the benchmark comparison on a pull request is posted as a new comment on every push, with the one it supersedes deleted, instead of being edited in place. A conversation orders its comments by creation time and an edit does not move one, so the table stayed where the pull request's first push had put it - above every commit and review comment made since, which is exactly where an out-of-date table would sit. Nothing failed when that happened: the job was green and the numbers were current, they just read as stale to everyone reviewing. There is still exactly one table per pull request, and it now names the head commit it measured and links the run that produced it, so a reader who arrives between a push and the next measurement can tell which commit the numbers describe. The new comment is posted before the old one is deleted, since the reverse order leaves the pull request with no comparison at all whenever the post fails, and ``tests/test_benchmark_workflows.py`` pins both that ordering and the absence of the in-place edit

* the offset table the lookup path addresses packed polygon payloads through is checked against the FlatBuffers reader for every ring in both coordinate files, by ``timezonefinder._data_integrity.validate_payload_offset_table`` - run by the converter over the files it just wrote and by the test suite over the files the package ships, like the hole reference checks next to it. The fast path assumes what the reader guarantees: that a table sits where its reference says, that vtables are aligned, and that the payload field is present. Nothing about a violated assumption announces itself, since an offset landing one word early still yields plausible packed residuals and therefore plausible wrong coordinates - so it is established exhaustively where the data is produced rather than defended cheaply where it is read. The format-3 frame and payload validator lives in the same installed module: the converter gives ``validate_block_payload`` the source rings for an exact round trip, while validation of an existing compiled directory re-derives every frame from the decoded coordinates

* ``prototypes/query_stage_profile.py``'s ``FINDINGS`` block is re-measured against the tree this release ships, on both acceleration backends and both coordinate access modes, since a change to the critical path moves every share quoted from it and a stale share reads exactly like a current one. The H3 resolution move does more than shift the shares: the ``unique`` and ``ambiguous`` strata are a classification *by* the shortcut index, so they are not the same points the previous block measured, and the block now reports all four strata rather than two - the uniformly random one being the workload-representative figure to rank on. Its fifth finding - the memory-mapped accessor costing 4.9 microseconds per candidate polygon - is now closed, and what remains of the gap to ``in_memory=True`` is recorded as a fresh one: ``np.frombuffer`` re-acquires the memory map's buffer on every fetch. The documented command for running it against the C extension gained ``--group test``; ``--isolated`` drops the default dependency groups, so it could not import ``tests/auxiliaries.py`` and had never worked as written

* the coordinate resolution this package claims is now derived rather than restated. ``timezonefinder.utils.coordinate_resolution()`` returns the representable step of any dtype - the ULP of a float, which depends on magnitude and is worst at ±180°, or the fixed-point step of an integer read as a scaled coordinate, which is the same everywhere - and ``degrees_to_metres()`` turns it into ground distance at a given latitude. ``tests/test_coordinate_precision.py`` holds every documented figure to what they compute, asserting the numeric band the prose commits to rather than its wording. Two claims were wrong and are corrected: ``configs.py`` called the 10\ :sup:`7` scaling "microdegree precision" when one unit is 10\ :sup:`-7` degrees, a *tenth* of a microdegree, and ``scripts/file_converter.py`` put the worst case at "1cm" when it is ~1.11 cm - the error is a whole step rather than half of one because the conversion truncates toward zero instead of rounding. ``docs/data_format.rst`` also records what the source data actually carries, which decides whether the stored precision is a ceiling or padding: timezone-boundary-builder publishes six decimal places, so the seventh the encoding provides carries nothing - measurable rather than assumed, since the last decimal digit of all 15,850,626 packaged values is only ever ``0`` or ``9`` and the nines are the conversion truncating toward zero. Relaxing it would not shrink the current layout or speed up a query, because ``int32`` is forced by the ±180° range rather than by the precision and no reachable resolution brings the globe inside ``int16``; it is worth ~18 % only to a variable-width encoding. ``docs/data_format.rst`` gains the comparison the input-dtype choice rests on: ``float64`` resolves ~3.5 million times finer than the packaged data and represents the scaled value exactly, while ``float32`` is ~153x coarser at the antimeridian (~1.7 m) and cannot hold 180 × 10\ :sup:`7` exactly at all. The usage docs warn callers accordingly - a ``float32`` coordinate array is accepted and upcast, but a point nearer a border than ~1.7 m can already have been rounded onto the wrong side of it

* two responsibilities the batch lookups had accumulated in ``timezonefinder/timezonefinder.py`` now live where they belong. Turning a zone id into a name - the string table, the lazily built gather array, both conversion regimes and the threshold between them - moves to ``timezonefinder/zone_names.py`` as a ``ZoneNames`` object, next to the file it reads: an id is an index into that list and nothing more, so naming one touches no coordinate, shortcut or polygon. ``timezone_names`` stays a documented public list, now a read-only view onto it. Admitting a batch of coordinates - ``coordinate_arrays``, ``out_of_bounds`` and the ``on_invalid`` policy set - moves to ``timezonefinder/utils.py`` beside the scalar ``validate_coordinates``, so the one place the two arities deliberately differ (a ``None`` coordinate, which ``float()`` refuses and ``numpy`` would turn into ``NaN``) is visible from both. The lookup module drops to the abstract base and the two finder classes. Measured paired and order-alternated on the tracked C-extension backend, the two estimators disagree in sign on both strata, so the added attribute hop on the name lookup is below what a query can resolve

* the candidate-polygon loop that resolves a point in a cell several zones cover exists once rather than twice, and what a cell costs to *prepare* for that loop - its candidate list, their zone ids and the index past which no other zone can match - is now separable from working it. ``timezone_at`` prepares one cell and works it; a batch prepares each distinct cell once and works every point that landed in it, memoised by shortcut entry rather than sorted by it, so a batch that shares nothing pays a dict lookup per point instead of an ``argsort``. Preparation is 898 ns against 10,228 ns for resolving a whole ambiguous point on the C extension, which is the ceiling on what sharing it can win. ``ShortcutIndex`` gained ``entries_of``, the batch sibling of ``entry_of``, so the batch path asks the index what a set of cells resolves to instead of indexing its table itself; it is pinned against ``entry_of`` over every cell at the shortcut resolution. ``TimezoneFinderL`` shares more than the preparation, since its answer is a property of the shortcut entry alone: its whole ambiguous resolution is one gather over the distinct entries, worth ~14% of a batch of such points. The answer dtype is ``int16``, the narrowest that holds a zone id, and ``timezonefinder._data_integrity.validate_shortcut_index`` now refuses a data directory whose zone count outgrows it - at build time and over the committed data, as for the widths beside it - so the width is chosen by fit rather than by headroom and cannot truncate silently. The single-point path pays one added method call for the shared loop, on the order of 1% of an ambiguous query - the price of not keeping a second copy of the stop-index and untested-last-zone logic, and bounded by ``test_batch_and_scalar_agree_over_every_committed_point``, which compares the two paths over every point of every committed fixture. Splitting preparation out cost it nothing further: measured paired and order-alternated over 244 rounds a side, the two estimators disagree in sign

* ``docs/benchmarking_methodology.rst`` gains two sections. One covers comparing two candidate implementations of a single stage, which the existing suites do not do and which three different measurement designs got wrong in a row - each of them flattering the newer candidate, by up to 13 percentage points. The other explains reading a stage's share of a query asymmetrically: it bounds what optimising that stage can win and says nothing about what redesigning it can lose, which for the shortcut lookup is at most a few percent of a mixed workload against a measured +29% for one plausible redesign. ``contributing/improvements/items/packaging-distribution-and-release/bench-1-the-pull-request-benchmark-comparison-cannot-resolve-the-changes-worth-reviewing.md`` records the tooling gap the same measurements exposed: the pull request benchmark comparison measures base and head in two sequential runs reduced to a single estimator each, so it cannot resolve the low-single-digit effects that three open performance items will need to demonstrate.

* a new prototype, ``prototypes/query_stage_profile.py``, attributes a ``timezone_at`` query to its individual stages - coordinate validation, H3 cell lookup, shortcut resolution, candidate bookkeeping, bounding-box rejection, hole tests and the point-in-polygon kernel - across both acceleration backends and both coordinate access modes, so the optimisations that were each justified by an assumption about where query time goes can be argued from a measurement instead. It takes each figure with three instruments chosen to fail differently - exact line hit counts scaled by a signal sampler's time share over the untouched function, a ladder of prefixes of the real lookup, and ``line_profiler`` for line ordering only - because no single one of them is unbiased at this scale: enabling ``line_profiler`` deoptimises the whole interpreter and inflates a pure-Python stage ~8x against ~1x for time inside the C extension, which skews exactly the geometry-versus-overhead ratio being measured. It is run by hand and records its findings in the file, like every other script under ``prototypes/``. Nothing in the package changes. Solves issue #497

* the artefact-staging block in ``build.yml`` is one composite action (``.github/actions/stage-artifacts``) instead of three near-copies. The copies had drifted - two unzipped ``artifact-*.zip`` and one ``artifact-*``, which silently unzips nothing because ``find -exec`` does not propagate its command's exit status - and the drift sat next to the one difference that is deliberate: the PyPI upload job must not stage ``timezonefinder_data-*`` wheels, since it publishes whatever is in ``dist/`` under the code project's trusted publisher while the data distribution releases from ``publish_data.yml``. That exclusion is now a named input rather than something a reader has to spot. In the same pass the ``publish-pypi`` job lost four steps that nothing had consumed since the data-dependency check moved into ``release``: it no longer sets up Python, upgrades pip, installs uv or reads a version number, and is now download, stage, publish. ``tests/test_release_workflows.py`` asserts that no job stages ``dist/`` inline, that the uploading job excludes the data wheel, and that every job using a local action checks the repository out first
...
* ``tests/`` is now type-checked by the mypy pre-commit hook, which had excluded it. While it was excluded eight real disagreements accumulated: the base test class in ``tests/main_test.py`` never declared the ``test_instance`` its fixture assigns and its tests read, and two of its class-level knobs were typed by inference from the base's own value, so the subclass naming ``TimezoneFinder`` and the one listing a third keyword-only method both contradicted a type nobody had written; ``built_files`` in ``tests/test_package_contents.py`` promised a ``list[Path]`` while the attribute behind it was only ever inferred as ``None``; ``tests/locations.py`` annotated a ``dict`` literal as ``set[str, str]``; and the memory fixture let an unmeasured metric through as ``None``, which would have failed as a ``TypeError`` naming neither the configuration nor the metric. The hook's stub list gained ``types-PyYAML``, and the guard that keeps ``scripts/`` out of the exclude pattern now covers ``tests/`` as well
...
* the data converter creates its ``boundaries/`` and ``holes/`` output directories once, in ``write_binary_files``, rather than in each of the two writers below it. The pair of writers is not independent - the reference vector the numpy writer emits addresses the coordinate file the FlatBuffers writer writes - so each creating the directories read as though either could be run on its own. The per-polygon zone ids now go through ``store_per_polygon_vector`` like every other vector, so that one file is no longer the only one written without its progress line. The compiled output is byte-identical
...
* the packaging guard now checks the metadata a build *produces* - ``PKG-INFO`` in the sdists, ``METADATA``/``WHEEL``/``RECORD`` in the wheels - for both distributions. Every case of the existing essential-files check names a path that exists in the checkout, so none of it could ever reach a file that has no counterpart there; missing metadata would have surfaced at upload instead, in a release whose version number cannot be reused
