...
* a new ``make bootstrap`` obtains the packaged boundary data instead of requiring the checkout to carry it: it fetches the ``timezonefinder-data`` release ``packages/timezonefinder-data/pyproject.toml`` declares, refuses a download whose SHA-256 disagrees with the digest PyPI publishes for it, and unpacks only the dataset. A second run does nothing, and a checkout that has moved to a commit declaring a different data version re-fetches - which is the case a "the directory exists" check cannot see, and would otherwise mean testing one release's data against another release's code with nothing to signal it. ``make test``, ``make testint``, ``make testall`` and ``make reports`` now depend on that check, and the test suite asks it once before collection, so a bare or stale data directory is reported as a sentence naming ``make bootstrap`` rather than as a ``FileNotFoundError`` on a ``.npy`` file three frames down. The data directory is still committed; nothing about installing or using either distribution changes
