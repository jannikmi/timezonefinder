...
* a new prototype, ``prototypes/shortcut_resolution_query_bench.py``, swaps a resolution 4 shortcut index underneath the real ``TimezoneFinder.timezone_at`` to settle whether the finer resolution pays for itself on a whole query, paired and order-alternated as this repository's methodology requires. Its timing comparison never ran: the correctness gate that precedes it found the two indices answering one fixture point differently, and resolution 4 is the wrong one there. The cause is pre-existing and unrelated to the index format - the shortcut compiler decides polygon/cell overlap by vertex inclusion alone and never tests whether a polygon edge crosses the cell, a simplification whose stated precondition (that polygons and cells have a similar size) degrades as the resolution rises. Recorded in ``potential-improvements.md`` as a blocker on any resolution change, together with a second, narrower gap it turned up at the poles, where cells above latitude 88 can omit the polygon covering them. Both are invisible to a realistic workload today: sampled by area rather than uniformly in latitude, 3,000 points produce no wrong answer at either resolution. Nothing in the package changes
...
* ``prototypes/query_stage_profile.py``'s ``FINDINGS`` block is re-measured against the tree that carries the coordinate offset table, on both acceleration backends and both coordinate access modes, since a change to the critical path moves every share quoted from it and a stale share reads exactly like a current one. Its fifth finding - the memory-mapped accessor costing 4.9 microseconds per candidate polygon - is now closed, and what remains of the gap to ``in_memory=True`` is recorded as a fresh one: ``np.frombuffer`` re-acquires the memory map's buffer on every fetch. The documented command for running it against the C extension gained ``--group test``; ``--isolated`` drops the default dependency groups, so it could not import ``tests/auxiliaries.py`` and had never worked as written
