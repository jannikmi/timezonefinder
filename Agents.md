...
- `CHANGELOG.rst` entries are not just for release-triggering or user-facing changes - internal/dev-tooling work (new scripts, test infrastructure, CI config, refactors) also needs a bullet under the unreleased `Internal:` sub-list. It is easy to skip this for a change that never touches `timezonefinder/` itself; do it anyway, as part of finishing the work rather than waiting to be asked.
